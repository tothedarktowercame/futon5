...
asymptotic activity (ordered rules score $0$, chaotic rules approach $1/2$, and
the complex rules---including Rule~$110$---fall between). Under this tint
(Figure~\ref{fig:tint}) the offset $+2$ field resolves into coexisting ordered
and chaotic domains separated by a definite boundary rather than a uniform
turbulence. Thinning that boundary to a filament skeleton recovers the
``separating curve'' directly from the data.
...
($D \approx 1.7$) than the feedforward offset $+2$ ($D \approx 1.5$), the
interface being where the coupling becomes visible. And it points at a sequel.
...
\caption{\textbf{The genotype as an order/chaos map.} The offset $+2$ genotype
field (two representative seeds, fixed-boundary variant) tinted by each rule's
intrinsic chaos score---its asymptotic activity as an isolated elementary CA
(blue $=$ ordered, red $=$ chaotic, pale $=$ complex, including Rule~110).
Ordered and chaotic domains coexist, separated by a definite interface.}
\label{fig:tint}
\end{figure}
