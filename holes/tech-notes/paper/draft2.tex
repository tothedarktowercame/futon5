...
fields. That surviving phase carries the visual signature associated with
the \emph{edge of chaos} \parencite{langton1990computation}, and a reader is
entitled to ask whether the resemblance is more than skin deep. It is, in a specific and measurable sense, and naming that sense is the purpose of this
section.

The concept ``edge of chaos'' can be understood in two senses. The first is \emph{global}: a system
tuned by a control parameter to a critical point between an ordered and a
disordered phase---Langton's $\lambda$, which \secref{sec:lambda} shows is
...
\secref{sec:lambda}. Ordered rules score near $0$; complex rules such as
...
critical transition; consistent with \secref{sec:limits}, the honest
