...
$\sigma=[0,0,1,2,3,4,5,6]$. The distinct-rule count (grey) falls from tens to
two; rules $170$ (red) and $42$ (dark) partition the field. Bottom: the later
one-write propagator idealisation, which applies the same $\sigma$ only once
after blending. Its terminal genotypes are plotted rather than assuming the
historical pair. This lower panel isolates the propagator cleanly, but is not a
