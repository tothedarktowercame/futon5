...
perturbation-spreading, entropy-rate, or correlation-length statistics. The
...
reading of the search that produced the family. The operator of
Figure~\ref{fig:regimes} that comes to be dominated by Rule~110---a universal
rule, the naive prize of a search for computational intelligence---does so by
concentrating about a third of the field onto it, and in that run holds fewer
distinct rules ($\approx 18$) than a foil that stays under $2\%$ Rule~110 yet
keeps a more diverse population ($\approx 23$). On this one pair, recovering a
universal rule and sustaining a diverse population pull against each
other---a suggestion worth testing at scale, not a law we have established.
