...
Four deterministic measurements settle why the stronger claim is not
available. The question decomposes into a search half and a holding half, and
both close.
...
exceeds $0.540$. The mechanism is a timescale mismatch: the settled field at a
site decorrelates in about six steps, less than half the $w = 15$ window that
gives the observable its discriminative power, so integration pools evidence
...
temporally integrated detector works. The run-level detector result is
untouched; it is per-decision use that is closed.

\emph{The holding half closes against a sharper surprise}
(Figure~\ref{fig:binv-rescue}). The natural local mechanism---let cells that
...
accelerates the phase it was built to resist. A variant that instead gives
...
cells, to within seed noise, so the placement carries nothing; and what either
preserves is a fragmented state whose settled domains have median width $2$,
against the width $4$ of the configuration in the valley, with the majority of
domains at two cells or fewer. Undirected rule noise keeps this lattice alive.
It does not keep it anywhere.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/binv-rescue.pdf}
\caption{\textbf{Local intervention at the freezing arm.} $\beta = 16$,
$\kappa = 0.1$, the pinned seed of Figure~\ref{fig:exo-bisection}; four runs
from identical initial conditions, frozen defined as unchanged for fifteen
steps. Left pair: the unmodified selection policy absorbs at $t = 1722$;
boundary-directed adoption---pure-neighbourhood cells copying the operator of
a cell at a phase boundary---absorbs \emph{earlier}, at $t = 1484$. Right
pair: one random operator write per dwell at pure-neighbourhood cells prevents
absorption, and so does the same number of writes at uniformly random cells;
across six seeds the two are statistically indistinguishable, and both hold a
fine-grained mixture (median settled-domain width $2$) rather than the
coexistence regime of Figure~\ref{fig:exo-lavalamp}. Bottom: frozen fraction
against time for the four runs shown.}
\label{fig:binv-rescue}
\end{figure}
...
We avoid the phrase \emph{transmissible but not heritable}. The exotype grid
