...
permutation-propagators admits exact classification rather than sampling. $P_\sig$ has a
fixed rule exactly when every cycle of $\sig$ is even, an all-even permutation with $k$
cycles has $2^k$ of them, and every fixed rule is balanced, with Langton activity
$\lambda = 1/2$. A coordinate taking one value across every fixed point cannot
discriminate among them, and a finite-size scan finds a broad crossover rather than a
critical point, so the classical parameter-space reading is closed off inside this
...
current. Cutting the phenotype-to-genotype edge roughly halves reach; a gain varying only
that currency, with spatial statistics matched, moves it smoothly; and sustained rule
diversity, mutation, refuges, niches and rule mobility do not. Making the coupling
endogenous recovers the same coordinate: a predicate blind to the phenotype reproduces
the dial, one reading it exceeds the dial, and one reading a frozen phenotype of matched
width and firing rate does not. Posed from inside, currency becomes a limit on
regulation---reach is computed from a twin run and so is unavailable to the system, and
internal observables predict an intrinsic objective at $R^2 = 0.73$ against $0.02$ for
reach. No self-driving search was obtained, and an objective scoring a fixed window ranks
configurations inverted against whether they survive; what separates them is persistence.
Measurement closes the search question: local evidence decorrelates faster than any
discriminating window closes, and the intermediate regime is the substrate's default,
threatened only by the optimizing selection layer itself.
This measure is one of causal propagation and not of dynamical regime.}  \keywords{cellular automata, rule evolution,
...
within a family large enough to enumerate is what the rest of Part~I does.
...
Two measures were tried before the causal one and neither separated the regimes. A
normalised compression distance on the rule field tracks how many distinct rules are
present rather than how they are arranged, and a block-entropy estimate over the
phenotype is dominated by the same quantity. Both therefore report diversity, which
\secref{sec:diversity-axis} shows is not the coordinate that governs propagation. That
failure is what motivates measuring a perturbation directly rather than summarising a
field.
...
Part~III changes the object twice more. It first moves from an architecture whose gain we
set to one that computes its own, and then from a fixed rewriting operator to one the
lattice carries per cell. Its two halves are measured differently. The endogenous-gate results are
finite-run causal measurements over a chosen family of predicates, not a
characterisation of conditional compositions in general; the exotype results are
persistence measurements on single trajectories, which is what the search's own failure
made necessary. What
...
Part~III is narrower still, and narrow in two places. Its gate measurements cover one pair
of constituents---unit-rate transport against holding---and one predicate family, agreement
within a centred circular neighbourhood. Thirteen conditioned gates at four widths support the width
...
Its exotype measurements are narrower again. The vocabulary instantiates twelve of the
$40{,}320$ permutations Part~I classifies, the original four were hand-picked for
unrecorded reasons, and no experiment varies $\sig$ across that space, so nothing measured
there is known to generalise across propagators. The three examples are single
configurations at one lattice width, reported with the seeds that disagree with them: at
the two points where the phases are closest to balanced, two seeds reach opposite outcomes,
and we therefore report a region in which the parameters do not determine the winner rather
than a critical point.

\paragraph{Future work.} The two extensions the gain result suggested have each moved,
and neither is closed. Making the gain endogenous is done: a predicate reading the lattice
sets it, and reach responds to whether what the predicate reads is current. Letting the
system set its own operating point was attempted and failed, and Supplement~5 records how.
An earlier controller adjusted a parameter from an internal error signal and failed on
leverage and stability; the later apparatus scored a surface and moved over it, but never
closed a loop, because no runtime-visible target survived validation. The remaining task is
therefore not to show that adjustment is possible in principle, but to find a target a
system can compute about itself that stays valid over the horizons on which persistence is
decided.
