...
\textbf{No operator has a single-byte attractor as an invariant \emph{set}.}
Support $\le 1$ occurs for 0.0\% of $\sig$; the observed minimum support is 4.
(Support $\le 4$: 11.5\%; $\le 32$: 54.1\%; $\le 64$: 88.0\%.)

\paragraph{A correction, since an earlier version of this subsection gave the
wrong reason.} We previously wrote that a fixed byte ``would require
$\bit{\sig(k)} = \lnot\,\bit{k}$ for all $k$ simultaneously, which an
always-invert operator forbids.'' An always-invert operator forbids that only on
\emph{odd} cycles; around an even cycle the alternating assignment closes
consistently. Section~\ref{sec:parity}'s theorem is exact here --- a Boolean
fixed point exists iff every cycle of $\sig$ is even --- so $27.34\%$ of
operators \emph{do} have fixed bytes, and the count is exactly $2^{k}$ for an
all-even $\sig$ with $k$ cycles. Enumerating all 256 bytes against
\texttt{gen/rule\-permute} itself confirms it: $\mathrm{rotate}{+}1$ (cycle type
$[8]$) has \textbf{2} fixed bytes, \texttt{01010101} and \texttt{10101010} --- the
alternating colourings, 85 and 170; $\mathrm{rotate}{+}2$ ($[4,4]$) has
\textbf{4}; $\mathrm{three{+}five}$ ($[5,3]$) and $\mathrm{id}$ ($[1^8]$), both
any-odd, have \textbf{0}. The two statements are not in conflict once separated:
fixed \emph{bytes} exist for all-even operators, while no operator's invariant
\emph{set} has support $\le 1$ --- because, as the next paragraph shows, the
propagator never runs long enough alone to reach one.
