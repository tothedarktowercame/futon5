...
This places every fixed point of every
operator at exactly the value the classical picture associates with the
transition between chaos and order.  However, in this setting,
$\lambda$ is powerless to distinguish an operator whose
rule field stays diverse from one that collapses. A
...
\emph{architectures} on this substrate, constructed from simple
operators including those mentioned above, along with temporal
schedules, phenotype-reading constructions, mutation, and conservative
transport. We use the reach of damage-spreading to characterise these
architectures, calibrated against rules of undisputed regime.  
Sustained rule diversity is an analogue of the order parameter from
the classical account, however, we develop a conservative control that holds
the rule histogram invariant while varying transport, and show that
damage-spreading can be effectively unchanged with regard to diversity
while moving by an order of magnitude with transport. Within the
tested architectures, differences in causal reach are driven by
genotype-to-phenotype \emph{coupling}. That coupling is a gain rather than a
switch: as the genotype's view of the phenotype moves from an ancestral copy to
the causally current one---marginal statistics matched throughout, so that only
currency varies---reach rises monotonically out of the ordered band and into the
complex one fixed by the elementary rules. How legible acquired state is to the
heritable layer is therefore a quantitative control on dynamical regime rather
than a categorical distinction. We then make that gain endogenous, replacing the
dial with a local predicate. A predicate blind to the phenotype reproduces the
dial exactly, linear in its firing fraction; one that reads the phenotype exceeds
it, by margins growing with the width of the neighbourhood read; and an otherwise
identical predicate reading a phenotype frozen at the perturbation time does not
exceed it at all. Since width is the spatial correlation length of the firing
pattern, correlated firing alone accounts for none of the gain: what a
self-setting coupling exploits is that what it reads is current.}  \keywords{cellular automata, rule evolution,
...
transition does not produce one (\suppfindingref{6}). The
...
while everything that reads it lands inside (\suppfindingref{10}). The coupling
...
Supplement~4 reports a separate exploratory analysis of
...
Part~III makes the gain of that coupling endogenous rather than imposed, and asks
whether a composition whose condition reads the lattice escapes the coordinate
Part~II identifies.
...
Figure~\ref{fig:map} maps that division of labour and marks where the scope
boundary falls.
...
\section{The 2015 Example}
\label{sec:warmup}

The operator classified in Part~I was reached by working out what a programming
error actually did. That derivation is short, it accounts for the figure that
motivated the enquiry, and it fixes the scope of everything that follows, so we
give it first.

The 2015 engine mutated a rule by reading the output at one displayed position
and writing its negation one position to the left, clamping at the end:
\[
  \bit{\max(k-1,\,0)} \;:=\; \lnot\,\bit{k}.
\]
Here $k$ indexes that engine's own display order, which is not the Wolfram order
adopted from \secref{sec:object} onward. The distinction does not affect the
argument of this section, which concerns only the shape of the map, but it does
affect any numerical comparison carried across the two conventions.

Two consequences follow at once, and both are visible in the engine's write log.
Position $7$ is never written---nothing shifts into the top---while position $0$
is written twice, once by $k=0$ and once by $k=1$. Over $87{,}632$ recorded
writes the measured shares are $0.000\%$ and $24.9\% \approx 2/8$. A map that
leaves one position without a preimage and gives another two is neither
injective nor surjective. It is not a permutation, and it is therefore not a
member of the $8!$ family classified below.

The same two properties account for the figure. Information cascades
$7 \to 6 \to \cdots \to 0$, negating at each step, so the configuration
consistent with the cascade is the alternating one, $g(k-1) = \lnot g(k)$.
Position $7$ receives nothing from this write and is determined by the remainder
of the update; positions $6$ down to $1$ then fill with alternation and freeze.
Position $0$ cannot settle at all, because $k=0$ demands $g(0) = \lnot g(0)$,
which no assignment satisfies. The field is therefore driven to exactly two
configurations differing in exactly one position, $00101010$ and $10101010$. Read
directly as eight-bit numbers these are the $42$ and $170$ reported with the
original figure---the genotype string itself, not a conversion into standard rule
numbering. The residual is a hunt on the single unsettleable position rather than
a pair of fixed configurations.

That distinction sets the scope of what follows. The mechanism just described
requires a position with no preimage and a position with two, and a permutation
has neither; no member of the $8!$ family can reproduce it. Part~I therefore
studies a neighbouring object, reached from this one and sharing its elementary
write, rather than a generalisation of it. The permutations are the bijective
core of the $8^8$ writings, and the 2015 example lies in the remainder.
...
enumerated, exactly classified family. The example in that 2015 paper is a
non-bijective writing (\secref{sec:warmup}) and therefore lies outside this
family; what it shares with the family is the elementary write, not membership.
We give the family here an essentially complete characterisation. The canonical symmetry group of ECA
...
survivors. The 2015 paper's Figure~8 exhibited a two-rule residual of its own: a
random field of tens of distinct rules dies off to an alternating population of
just two. That resemblance is not an instance of this count. The writing in that
example is non-bijective (\secref{sec:warmup}); the count above is a statement
about permutations; and that writing admits no fixed rule whatever, its residual
being a hunt on a single unsettleable position rather than a pair of fixed
points. Two-rule residuals arise here for a different reason than they do there,
...
found no additional structural regime.\supptheory{Definition~6}{Interrupter} Temporal braiding is a different
operation: it alternates stochastic elementary writes inside the spatially
coupled dynamics rather than replacing them by one composed map.\suppfinding{5}{The rotation split occurs with and without neighbour blending}
...
the complexity and compression measures we tried read a single rule's bits, or
one space--time trajectory, without explicitly encoding $\sig$. In our tests
none of them recovered the parity classification. A population count reads a
collapsed-but-maximally-selected field as nearly dead. A frozen periodic
genotype scores a lower statistical complexity $C_\mu$
\parencite{crutchfield1989inferring} than rule~110. And gzip rates a
self-similar rule as noise. We report this as a limitation of the summaries we
tested rather than as a finding: it is a negative result about those particular
measures, not an impossibility theorem for trajectory-based inference, and
unlike the findings collected in Supplement~1 it rests on exploratory runs we
did not preserve in reproducible form.
...
protocol of \suppfindingref{9}, with regime bands drawn from the elementary
...
Figure~S3.2 the empirical neighbourhood distribution is
...
\part{Endogenous Coupling}
\label{part:endogenous}

\noindent In Part~II the gain of the phenotype-to-genotype loop is a parameter we
set. This part asks what happens when it is not: when the coupling is a function
of the lattice's own local state rather than a dial turned from outside. The
objects are still compositions of operators Part~I classifies, so nothing new is
introduced at the level of the substrate; what changes is who sets the gain.
...
holds and another where it does not: $\mathrm{switch}(c, A, B)$. Throughout this
part $A$ is conservative transport at unit rate, which sits in the chaotic band
of the calibration of Section~\ref{sec:causal} at $25.38$, and $B$ is
holding, which sits in the ordered band at $1.21$. The predicate decides, cell by
cell and step by step, which applies. Its \emph{firing fraction} $f$---the
proportion of opportunities at which $A$ is selected---is measured from the run
rather than prescribed.

Two disciplines make the comparison below meaningful rather than merely
suggestive, and both are inherited from the gain dial of
Section~\ref{sec:gain}. First, the transport stream advances once at every
opportunity whether or not the gate fires, so a predicate that reads the
phenotype cannot desynchronise the two branches of the damage fork and
manufacture divergence that is not causal. Second, where the predicate consumes
randomness of its own, those draws come from a separate stream re-seeded
identically in both branches, so the gate never itself injects difference.

The first question is what a composition does when its condition carries no
information about the lattice. A gate firing at a prescribed rate, blind to the
phenotype, is well described across six rates spanning $f = 0.10$ to $f = 0.94$
by a line through the holding value,
%
\begin{equation}
...
\end{equation}
%
with the unconstrained fit returning $R^{2} = 0.993$. A blind conditional
composition is therefore \emph{exactly} a gain dial: its duty cycle plays the
role $\gamma$ plays in Section~\ref{sec:gain}, and reach is linear in it. This is
worth stating plainly because it is the null against which everything else in
this part is measured, and because it means the construction adds nothing on its
own. Making the gain a function of \emph{something} does not by itself buy
anything; it matters what that something is.

\section{A Reading Gate Exceeds the Dial}
\label{sec:reading-gate}

Now let the predicate read the phenotype. Write $\mathrm{agree}\,k/m$ for the
condition that at least $k$ cells of the centred circular $m$-cell phenotype
neighbourhood agree with the centre. Thirteen such gates were placed on the same
scale, spanning widths $m \in \{3,5,7,9\}$ and, at each width, strictness from
bare majority to unanimity.

Every one of them exceeds the blind line of Equation~\ref{eq:blind-line} at its
own measured firing fraction, and several do so by a wide margin. The largest
departure is $+22.8$ cells; the widest permissive gate, $\mathrm{agree}\,5/9$,
reaches $38.23$ while firing on only $63\%$ of opportunities---not merely above
the blind line but above unit-rate transport's own $25.38$. Firing a chaotic
operator less often, in the right places, spreads damage further than firing it
always.

What predicts the size of the departure is the \emph{width} of the neighbourhood
the gate reads, not how hard the gate is to satisfy. Adding width to a
quadratic-in-$f$ model of departure raises $R^{2}$ from $0.84$ to $0.96$, at
$+1.47$ cells per cell of width; adding strictness instead raises it only to
$0.88$. The design separates the two---the correlation between width and
strictness across the thirteen gates is $+0.01$---whereas strictness is nearly
collinear with the firing fraction itself at $-0.91$, so its apparent effect is
largely $f$ under another name. The model-free version of the same statement is
that within each of three groups of gates matched on $f$, departure increases
monotonically with width; in the group near $f \approx 0.65$ the strictness ratio
is almost constant at $0.56$--$0.60$ while the width nearly doubles.

\section{Currency, Not Correlation}
\label{sec:currency}

That result admits a deflationary reading, and it is the reading its own
mechanism suggests. A gate reading $m$ cells has neighbouring decisions sharing
$m-1$ inputs, so width \emph{is} the spatial correlation length of the firing
pattern. The blind gate of Equation~\ref{eq:blind-line} draws an independent coin
per cell and is therefore spatially uncorrelated. Correlated firing of a
\emph{transport} operator composes adjacent swaps into longer-range movement, so
a departure that grows with width might be pure geometry, with the phenotype
playing no part beyond supplying a conveniently correlated field.

The control that separates the two possibilities holds the phenotype still. A
\emph{frozen} gate applies the identical $\mathrm{agree}\,k/m$ predicate to the
phenotype as it stood at $t^{*}$: same predicate, same width, same spatial
statistics, a real phenotype field, and therefore the same correlation structure
in its firing. What it cannot do is track where the perturbation has since
spread. Each branch of the damage fork keeps its own $t^{*}$ snapshot, so both
frozen fields contain the perturbation itself and only the live gate sees its
consequences---the ablation of Section~\ref{sec:causal} applied to the
gate rather than to the update.

Across seven pairs matched on $(m,k)$, the live gate exceeds its frozen
counterpart in every case, by $14.5$ cells on average. More tellingly, every
frozen departure from the blind line is \emph{negative}, between $-0.04$ and
$-4.18$. A gate with the same width and the same correlated firing, reading a
field that is merely stale, sits at or slightly below the spatially uncorrelated
baseline. Spatial correlation on its own buys nothing here; if anything it costs
a little. The comparison is also conservative in the direction that matters: the
frozen gate fires \emph{more} often than its live counterpart in six of the seven
pairs, so it is given more opportunities and still reaches far less.

The reading we take from this is that the gate is a targeting mechanism, and
targeting works only on current information. Firing transport where the
phenotype is \emph{now} locally uniform lifts causal reach far above a blind gate
at the same rate; firing it where the phenotype \emph{was} uniform at $t^{*}$
does not, though the two firing patterns share width, spatial statistics and
rate. So the coordinate that governed in Part~II governs here too, reached by a
different route: an endogenous gain does not escape the causal currency of what
the genotype reads, it re-derives it. A composition whose condition is blind
reproduces the dial exactly; a composition whose condition reads the phenotype
exceeds the dial precisely to the extent that what it reads is current.

\part*{Discussion}
\label{part:discussion}
\addcontentsline{toc}{part}{Discussion}

\section{Specifying an Experiment Before Running It}
\label{sec:apparatus}
...
\subsection{The target as a certificate}
...
\subsection{Conditions checkable before the run}
...
This distinction is not hypothetical here. Under the gain parameterisation used in
\secref{sec:endogenous-gain}, sampled at eight uniform levels, the band score is zero
...
\subsection{Invariants the implementation must satisfy}
...
\subsection{Diagnosing a negative result}
...
\subsection{What the apparatus does not establish}
...
The paper has studied three related objects, and its conclusions must keep them
...
refuge probability, niche width and rule mobility, causal reach never leaves the
ordered band, and blend strength alone reaches its edge. The substrate carries no
fitness, so this concerns what the heritable layer can see and not what is
selected for.

Part~III changes the object once more, from an architecture whose gain we set to
one that computes its own. Its results are narrower than Part~I's and of the same
kind as Part~II's: finite-run causal measurements over a chosen family of
predicates, not a characterisation of conditional compositions in general. What
they establish is a negative and a positive that belong together. A composition
whose condition is blind to the phenotype reproduces the Part~II dial exactly,
linear in its duty cycle---so conditional composition is not by itself a new
mechanism. A composition whose condition reads the phenotype exceeds that dial,
by margins that grow with the width of the neighbourhood read; but an otherwise
identical gate reading a phenotype frozen at $t^{*}$ does not exceed it at all,
and in every tested pair falls slightly below. Width is the spatial correlation
length of the firing pattern, so this is the comparison that matters: correlated
firing alone buys nothing, and what the reading gate exploits is that what it
reads is current. The coordinate is therefore the same one Part~II identifies,
recovered from a construction that shares none of Part~II's dials. That
convergence is the strongest connection the paper draws between its parts, and it
is still a convergence of measurements rather than a theorem. Supplement~4 records the separate spatial-domain
analysis and the withdrawn boundary statistic; neither is a premise of this
two-part argument.

Part~III is narrower still. Its measurements cover one pair of constituents---unit-rate
transport against holding---and one predicate family, agreement within a centred
circular neighbourhood. Thirteen conditioned gates at four widths support the width
gradient; seven matched pairs support the frozen control. Each row is sixteen seeds at
one lattice width, so the between-seed spread is real and reported rather than
smoothed. Two limits deserve naming. The frozen gate's firing fraction does not match
its live counterpart exactly, differing by up to $0.04$; the discrepancy runs in the
direction that weakens the result, since the frozen gate mostly fires more often and
still reaches less, but it is not zero. And while the frozen control shows that current
information is \emph{necessary} for the departure, it does not decompose the width
gradient: how much of that gradient is currency and how much is the geometry of
correlated firing is not resolved here. A blind gate with matched correlation length,
rather than matched rate alone, would separate them and has not been run.

\paragraph{Future work.} Part~III closes the first of the two extensions the gain
result suggested and sharpens the second. The gain is no longer only a parameter we
set: a predicate reading the lattice sets it, and reach responds to whether what the
predicate reads is current. What that leaves open is the step from a gate to a
regulator. The conditions tested here are fixed families chosen by us; nothing yet
adjusts its own condition in response to how it is doing. The 2014 ancestor of this
substrate already contains the relevant construction---a switch that fires only where
the local context carries no new information---so a system that tunes its own gate is
buildable on this lattice rather than hypothetical, and the question is whether such a
system settles anywhere in particular on the calibrated scale.
...
population of these architectures would supply that ingredient, and because both arms
would then be expressible in one family they could be placed on the single measure used
throughout this paper: the reach of damage-spreading, calibrated against rules of
undisputed regime. Whether direct write-back and selection-mediated assimilation arrive
at the same place on that scale, and by what route, is a question the family is now
equipped to ask.
