...
than a categorical distinction, and a substrate able to regulate that gain would
hold an actuator on its own.}  \keywords{cellular automata, rule evolution,
...
selected for. Supplement~4 records the separate spatial-domain
analysis and the withdrawn boundary statistic; neither is a premise of this
two-part argument.

\paragraph{Future work.} Two extensions follow from the gain result, and the
second is the more interesting. The gain is imposed from outside here: $\gamma$ is
a parameter we set, not a quantity the system controls. The 2014 ancestor of this
substrate already contains a construction that makes it endogenous---a switch
that fires a propagator only where the local context carries no new information
and holds otherwise, so that write-back is state-dependent and requires no
edge-of-chaos oracle. Its behaviour differs from both of its unconditional
constituents, preserving rule diversity that unconditional firing destroys while
remaining more quiescent than holding. Placing $\gamma$ under that kind of local
control would test whether a system can locate the complex band unaided rather
than being tuned into it, which is the difference between a dial and a regulator.

The substrate also makes available a comparison that is usually only possible
across different models. As it stands it instantiates the Lamarckian arm alone:
acquired state writes into the heritable layer, and $\gamma$ is the strength of
that write-back. The Baldwin effect requires something this substrate lacks---a
selective filter, so that plasticity alters what selection sees and the heritable
layer follows without direct transcription. Adding selection over a population of
these architectures would supply that missing ingredient, and because both arms
would then be expressible in one family, they could be placed on the single
measure used throughout this paper: the reach of damage-spreading, calibrated
against rules of undisputed regime. Whether direct write-back and
selection-mediated assimilation arrive at the same place on that scale, and by
what route, is a question the family is now equipped to ask.
