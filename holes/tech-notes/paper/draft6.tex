...
by an order of magnitude with transport. Placed on that same scale the constructions
separate instead by \emph{coupling}: where the genotype update never reads the
phenotype nothing clears the bottom of the complex band, whatever the dial,
while everything that reads it lands inside. Treating that coupling as a
quantity rather than a property recovers an order parameter---reach is graded
and monotone in the gain of the phenotype-to-genotype loop, crossing into the
complex band at a locatable value---while a control matched in rule mobility but
blind to the phenotype stays ordered throughout. Adding the
...
the search failed, and where the order parameter had been hiding. The
...
while everything that reads it lands inside (Empirical
finding~\ref{find:architecture}). The coupling is moreover a dial rather than a
switch. Tuning the gain of that loop carries reach smoothly from the frozen
...
the phenotype, never leaves the ordered band. The classical picture is therefore
recovered, in a coordinate it does not use: not a parameter of the update, but
the gain of the loop from the phenotype back to the genotype.
...
Empirical finding~\ref{find:ladder}. \textbf{(a)} Constructions split by
