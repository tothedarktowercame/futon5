...
bare operator family and every parameter dial we swept fall in the \emph{ordered}
band, most at or below the frozen identity---the genotype update is feedforward,
so a phenotype perturbation never reaches the rule field at all. Adding the
...
the search failed. The bare operator family and every dial we swept---$\lambda$,
propagator fraction, sustained diversity, blend strength, mutation rate, refuge
probability, niche width, conservative transport rate---land in the
\emph{ordered} band, most at or below the frozen identity (Empirical
finding~\ref{find:architecture}). The reason is structural rather than
parametric: the genotype update is feedforward, reading the rule field and its
own tape but never the phenotype, so in the bare family a phenotype perturbation
cannot reach the rule field at all. Adding one edge---letting the phenotype be
read back into the genotype---carries the same construction into the complex
band, and cutting it carries the construction back out.
...
\begin{empiricalfinding}[label={find:architecture}]{Causal reach is conferred by the feedback edge, not by any parameter of the family}
Run under the protocol above---$L=80$, $t^{*}=60$, one phenotype flip, damaged
phenotype cells at $dt=59$---every configuration of the bare operator family
lies in the ordered band. The bare operator $P_a$ reaches $0.03$ cells; braided
writings $0.12$; asynchronous refuges $0.20$--$1.02$; mutation $0.23$--$0.85$;
the preserving limit $1.10$; rot$+1$ $2.05$; and the strongest of them, the
braid of rot$+2$ with rot$+4$, $5.20$. All sit below rule $90$ ($8.00$) and most
below rule $204$ ($1.00$), the frozen identity, whose flip persists as exactly
one cell and never spreads. A bare-family perturbation does not merely fail to
spread; it usually vanishes.

This is a consequence of the architecture rather than a limitation of the dials.
The genotype update is feedforward: it reads the rule field and its own tape,
never the phenotype. In the bare family a phenotype perturbation therefore
cannot reach the rule field \emph{at all}, and propagates only within the
phenotype layer, under a rule field that is rich, moving, and wholly indifferent
to it. The river construction of \secref{sec:braiding}, which closes that loop,
reaches $12.97$---between rules $90$ and $54$. Cutting that single edge returns
it to $5.51$, level with the strongest bare mechanism
(Figure~\ref{fig:regime-placement}).

What separates ordered from complex behaviour here is thus not the value of any
coordinate we have swept---$\lambda$, propagator fraction, sustained diversity,
blend strength, mutation rate, refuge probability, niche width, or conservative
transport rate---but whether the phenotype-to-genotype edge is present at all.
\end{empiricalfinding}

\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figures/regime_placement.pdf}
\caption{\textbf{Causal reach is conferred by the feedback edge.} Every point is
run under one protocol ($L=80$, $t^{*}=60$, a single phenotype flip, damaged
phenotype cells at $dt=59$), so the family and the elementary rules share an
axis; bands are drawn from the elementary rules alone. The calibration
reproduces rules $0$, $204$ and $90$ exactly against the values of
Empirical finding~\ref{find:ladder}. The entire operator family and every
parameter dial fall in the ordered band, most at or below the frozen identity.
The river construction (red) reaches the complex band; with its
phenotype-to-genotype edge cut (grey) it falls back beside the strongest bare
mechanism. The gap between the two stars is the paper's edge of chaos, and it is
crossed by adding an edge, not by tuning a parameter.}
