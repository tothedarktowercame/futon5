...
The edge of chaos is nonetheless present---but not as a coordinate. Flipping one
cell and tracking divergence against an otherwise identical run places a
phenotype-reading construction between elementary rules $90$ and $54$ on a
damage-spreading scale calibrated against rules of undisputed regime. Sustained
rule diversity looked like the order parameter the classical account wants; a
conservative control that holds the rule histogram exactly invariant while
varying transport shows that it is not, reach being flat in diversity and moving
by an order of magnitude with transport. Placed on that same scale, however, the
constructions separate by coupling rather than by parameter: where the genotype
update never reads the phenotype nothing clears the bottom of the complex band,
whatever the dial, while every construction that does read it lands inside. Adding the
single phenotype-to-genotype edge carries the construction into the complex
band; cutting it carries the construction back. In this family the edge of chaos
is crossed by adding an edge, not by tuning a parameter, which is why no
parameter scan located it.}
...
transition does not produce one (Empirical finding~\ref{find:crossover}). The
...
Placing every one of those coordinates on the calibrated scale then shows why
the search failed. The constructions separate by \emph{coupling}, not by
parameter. Where the genotype update never reads the phenotype---the bare
operators, braids, niches, refuges, mutation, and blend strength across its
whole range---nothing clears the bottom of the complex band (Empirical
finding~\ref{find:architecture}). Where it does read the phenotype, everything
lands inside: the river construction, and a conservative transport whose swaps
are gated on the local phenotype interface, which reaches past rules $110$ and
$54$. The reason is structural. A phenotype perturbation that cannot reach the
rule field propagates only through a layer indifferent to it. Adding one
edge---letting the phenotype be read back into the genotype---carries the same
construction into the complex band, and cutting it carries the construction back
out.

This is the paper's edge-of-chaos claim, and it is singular. The edge is real,
it is measured rather than assumed, and it separates ordered from complex
behaviour as sharply as anything in the classical picture. It is simply not
located at a value of a parameter, which is why no parameter scan could find it.
It is crossed by adding an edge.

An accompanying supplement reports a separate exploratory analysis of
...
This paper presents a classification of the operators, a census of the spatially
coupled dynamics across all cycle types, the finite-size search for a critical
transition, and the causal measurement that produced the edge.
The exact results hold for the entire family. The empirical claims are narrower
and are marked as such: the causal measurement covers one construction at one
lattice width over six seeds, and no comparable measurement of the bare operator
family is attempted here.
...
existence, number, and balance from the operator definition. Balance forces the
rest: with $\lambda = 1/2$ at every fixed point of every operator, the classical
order parameter cannot discriminate within this family, so an edge of chaos must
be sought in another coordinate. The empirical branch (amber) pursues the
parameter-space reading and finds no critical point; the constructive branch
(green) pursues the dynamical reading, sustaining structure by braiding and by
phenotype-to-genotype feedback and measuring how far a perturbation travels.
Both converge on a causal test of the candidate diversity coordinate, which the
conservative-flow control rejects as an order parameter. Arrows mark logical or
methodological dependence. Exploratory material on spatial domains, and a
withdrawn boundary statistic, are reported in the supplement and are not
premises here.}
\label{fig:map}
\end{figure}
...
report that structure in the accompanying supplement, together with a measure of
...
The failure above is not peculiar to our estimator. Any statistic computed over
a region drawn from the field it is computed on invites it, and no null defined
on the region can detect it. The repair is to stop drawing regions: perturb one
cell and ask where the effect arrives.

We run the phenotype-reading construction of \secref{sec:braiding} to $t^{*}=60$,
...
\begin{empiricalfinding}[label={find:architecture}]{Causal reach tracks the presence of the feedback edge, not the value of any parameter}
Run under the protocol above---$L=80$, $t^{*}=60$, one phenotype flip, damaged
phenotype cells at $dt=59$---the constructions divide by \emph{coupling}, not by
which parameter they vary. Where the genotype update never reads the phenotype,
no construction clears the bottom of the complex band. That covers the bare
operators ($P_a$ $0.03$, rot$+1$ $2.05$), braided writings ($0.12$ and $5.20$),
spatial niches ($0.50$, $0.72$), asynchronous refuges ($0.47$, $0.75$),
replacement mutation ($0.23$, $0.85$), the preserving limit ($1.10$), and blend
strength across its whole range---$1.00$ giving $0.03$, $0.70$ giving $0.55$,
$0.35$ giving $3.38$, and $0.00$, the most propagating feedforward
configuration we found, giving $8.15$, level with rule $90$. Most sit below rule
$204$ ($1.00$), the frozen identity, whose flip persists as exactly one cell and
never spreads; the bare operator's usually vanishes outright.

Where the genotype update \emph{does} read the phenotype, every construction
lands inside the complex band. The river reaches $12.97$, between rules $90$ and
$54$. The conservative transport of \secref{sec:diversity-axis}, whose swap
probability is gated on the local phenotype interface, reaches $19.27$ at rate
$0.50$ and $25.38$ at rate $1.00$---past rules $110$ and $54$, and approaching
the chaotic rule $30$ ($36.45$). Cutting the river's single edge returns it to
$5.51$, back among the feedforward constructions
(Figure~\ref{fig:regime-placement}).

The separation is clean: no feedforward construction we tested exceeds $8.15$,
and no construction that reads the phenotype falls below $12.97$. This is not a
statement about the permutation-propagator family in particular---blend
strength, refuges, niches and conservative transport are modifications of the
update rule, not members of it---but about coupling. A phenotype perturbation
that cannot reach the rule field propagates only through a layer that is
indifferent to it. What separates ordered from complex behaviour here is
therefore not the value of any coordinate we swept---$\lambda$, propagator
fraction, sustained diversity, blend strength, mutation rate, refuge
probability, niche width, transport rate---but whether that single edge is
present.
\end{empiricalfinding}
...
\caption{\textbf{Causal reach tracks the feedback edge.} Every point is run
under one protocol ($L=80$, $t^{*}=60$, a single phenotype flip, damaged
phenotype cells at $dt=59$), so all constructions share an axis with the
elementary rules, from which the bands are drawn; the calibration reproduces
rules $0$, $204$ and $90$ exactly against
Empirical finding~\ref{find:ladder}. Rows separate constructions by
\emph{coupling} rather than by provenance. No construction whose genotype update
never reads the phenotype clears the bottom of the complex band, the most
propagating being blend strength $0.00$ at $8.15$; every construction that does
read it lands inside, including phenotype-gated conservative transport at
$19.27$ and $25.38$. The river appears in both rows---at $12.97$ with its edge
intact and at $5.51$ with the edge cut---so the arrow between the rows is the
same construction, moved by adding one edge rather than by changing a
parameter.}
...
collapses. The sustained distinct-rule count can, and the causal measure above
...
describes whether this operator family sustains a population, but it does not
govern causal reach. The latter depends on how rewriting or transport is
organised. Identifying a coordinate that captures that organisation across
mechanisms is work for another paper; the classification of
\secref{sec:classification} guarantees that $\lambda$ is not it.
...
the supplement failed. It concerns the phenotype-coupled construction, not the bare operators;
...
fixed-boundary variant), and the spatial domain structure is reported in the
supplement rather than here. The information measures of that supplement
are withdrawn: their mask and their measure are derived from the same
thresholded field, the three nulls we ran cannot detect that, and measures which
avoid it return nothing. We report the failure in full because the same
construction is easy to reproduce and the nulls that would seem to guard against
it do not. The causal measure of \secref{sec:causal} replaces it, and carries
its own limits: one construction rather than the operator family, one lattice
width, six seeds, and an exponent fitted over a range where the damage width
reaches only a tenth of the lattice, so finite-size effects are not excluded.
...
A third strand set out to ask whether the filamentary boundaries of
\secref{sec:discussion} carry information, and returns a negative. The statistic
we used is an artefact of drawing the region from the field the statistic is
computed on, and the geometry-preserving nulls that appear to guard against that
cannot---they displace the mask off exactly the cells that produce the effect.
We report it because the construction is a natural one to reach for and the
failure is invisible from inside it. What replaces it is causal rather than
observational: perturbing a single cell and tracking the divergence against a
matched control shows that the phenotype-reading construction roughly doubles
how far a perturbation travels, and places it between elementary rules $90$ and
$54$ on a scale calibrated against rules of undisputed regime. That is a
narrower claim than the one we set out to make, and it is the one the evidence
supports.
