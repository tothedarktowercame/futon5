...
The edge of chaos is nonetheless present, in a coordinate that account does not
use. Flipping one cell and tracking divergence against an otherwise identical
run places a phenotype-reading construction between elementary rules $90$ and
$54$ on a damage-spreading scale calibrated against rules of undisputed regime;
cutting its single phenotype-to-genotype edge roughly halves how far a
perturbation travels. Across seventy-six configurations and seven mechanisms,
causal reach appears to rise with sustained rule diversity and then fall, but
every point on the descending limb obtains high diversity through mutation or
by holding rules still. A conservative control removes that confound: local
bijective exchanges preserve prescribed rule measures spanning $32$--$256$
effective rules while keeping the field mobile. At fixed exchange rate reach is
approximately flat across that range, whereas changing the exchange rate
changes amplification by more than an order of magnitude. Diversity is
therefore descriptive, not an order parameter; structured genotype transport is
