...
change. Measuring the row-to-row rewrite rate of the rule field by region, the wall
churns more than either domain interior---for offset $+2$, $0.86$ at the wall
against $0.80$ in the ordered interior and $0.63$ in the chaotic one, an ordering
(wall $>$ ordered $>$ chaotic interior) that holds for every seed we ran and for
the river and $\sigma = 16250374$ alike. The inversion is the content: the chaotic
domain, active in \emph{phenotype}, is the most \emph{genotype}-stable, while the
wall---where an ordered domain abuts a chaotic one---is where the propagator does
its most rewriting. The walls are not compositionally special: no rule class is
enriched there, and the universal Rule~110 is if anything depleted, so the
