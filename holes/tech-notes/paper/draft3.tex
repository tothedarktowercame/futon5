...
It is the coupling that reverses this for the $\gcd 2$ case, and that
reversal---a fixed-point-bearing operator that nonetheless sustains a
high-diversity field---is what the survey displays.

Every non-trivial rotation is all-even and so possesses fixed points
(Theorem~\ref{thm:parity}); yet among them only offset $\pm 2$ (two $4$-cycles)
sustains, while the remaining rotations ($\gcd 1$ and $\gcd 4$) collapse to
near-uniform fields within a few dozen generations. This is a statement
\emph{within} the all-even rotation family: an operator carrying an odd cycle has
no fixed rule to settle onto and stays diverse for a different reason
(Figure~\ref{fig:parity}, right).
\end{workedexample}
...
\paragraph{Interfaces, and what the dimension does not capture.}
Across the tested widths, box-counting estimates for these spacetime boundaries
(box sizes $2$--$64$; a handful of seeds; the fixed-boundary variant) stabilise
near $D \approx 1.5$ for offset $+2$ and $D \approx 1.7$ for the phenotype-reading
river of Figure~\ref{fig:river}: evidence for a \emph{fractal-like, size-stable}
morphology over the scales measured, not a claim of exact scale invariance
(Figure~\ref{fig:interface}). \discussnote{how strongly to state the
offset$+2$-versus-$\sigma$ contrast: under the reproducible activity score the
regime-entropy gap is a \emph{modest} $1.46$ vs $1.23$ bits, not the sharp $1.45$
vs $0.92$ of an earlier ad-hoc score; keep the softened wording below, or drop the
entropy number and rest on the visual plus the diversity result?} The boundary
dimension is a shared morphological descriptor rather than a discriminator between
operators: offset $+2$ and $\sigma = 16250374$ carry boundaries of \emph{similar}
dimension ($D \approx 1.55$ for both). \discuss{What visibly separates them is the
organisation of the field the boundary bounds (Figure~\ref{fig:tint}): offset $+2$
resolves into large coexisting domains of distinct activity, whereas
$\sigma = 16250374$---which concentrates on the universal Rule~110 and pays for it
in sustained diversity (\secref{sec:conclusion})---is more uniformly mixed at fine
scale. A coarse three-way regime entropy registers this only modestly ($1.46$
versus $1.23$ bits), not as a sharp separation: the difference is one of domain
organisation, more legible to the eye than to any single scalar we have measured
for it.}
...
dominated by the intermediate complex Rule~110 (\discuss{$\approx 45\%$ of the
late-time genotype here; the share is seed- and engine-dependent, and
Figure~\ref{fig:regimes} reports a lower $29$--$35\%$ measured over the whole
field with the legacy engine}). The score orders the regimes but does not by
...
\caption{\textbf{The spacetime activity boundary is fractal-like and size-stable
for the sustaining operators.} Top: the boundary (black) between low- and
high-activity regions traced through the $1{+}1$-dimensional spacetime diagram at
$L = 256$. offset $+2$ carries a coherent boundary (box-counting
$D \approx 1.54$); offset $+4$ has collapsed (no boundary); the phenotype-reading
river carries a denser one ($D \approx 1.71$). Bottom: box-counting dimension
(box sizes $2$--$64$) against width $L$; over the tested range the estimate is
stable rather than drifting. Exploratory: a handful of seeds, fixed-boundary
variant.}
\label{fig:interface}
\end{figure}
