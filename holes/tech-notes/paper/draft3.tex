...
\paragraph{Interfaces, and what discriminates the interesting case.}
...
(Figure~\ref{fig:interface}). The boundary dimension is not, however, what
distinguishes the visually interesting case. offset $+2$ and $\sigma = 16250374$
carry boundaries of \emph{similar} dimension, yet differ sharply in regime
balance: the three-way activity mixture has entropy $1.45$ bits for offset $+2$
(a genuine coexistence) against $0.92$ bits for $\sigma = 16250374$, whose field
is dominated by the single complex Rule~110 (\secref{sec:conclusion}). It is the
\emph{balance} of regimes, not the fractal dimension of their boundary, that
separates a structured crossover from single-regime dominance.
...
dominated by the intermediate complex Rule~110 ($\approx 48\%$;
...
$D \approx 1.53$); offset $+4$ has collapsed (no boundary); the phenotype-reading
river carries a denser one ($D \approx 1.69$). Bottom: box-counting dimension
