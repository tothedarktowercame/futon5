...
(Figure~\ref{fig:churn}), the wall churns more than either domain interior---for
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.82\linewidth]{figures/eoc_churn.png}
\caption{\textbf{The genotype is rewritten fastest at the activity-domain walls.}
Row-to-row rewrite rate of the rule field (fraction of cells whose ECA rule
changes from the row above), by region, at $L = 256$ (mean $\pm$ sample SD over
the committed seeds). For every operator the wall exceeds \emph{both} the ordered
and the chaotic domain interior; the chaotic interior---most active in
phenotype---is the most genotype-stable. Wall extraction is the threshold-and-smooth
procedure of Figure~\ref{fig:interface}; reproducible from the committed fields
