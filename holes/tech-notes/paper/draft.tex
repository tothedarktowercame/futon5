...
of an operator's fixed-point condition. Operators with all-even cycle
structure possess attractors, and every attractor lies at the edge of
chaos; operators with any odd cycle possess none.}
...
Equation~\eqref{eq:prop} does not appear in it. Rule-space permutation has
surfaced only as a one-off illustration --- \textcite{pavlic2014picard}
swap two bits in a single figure --- never as an enumerated family with a
derived fixed-point structure. Two established genres are adjacent but
distinct: per-cell rules (non-uniform or $\nu$-CA
\parencite{dennunzio2012nonuniform}) and rules that change during evolution
(rule-changing or programmable CA \parencite{nandi1994rulechanging}). A
...
\parencite{dennunzio2012nonuniform}, so the substrate is not where novelty
...
Voorhees's $\Theta$. There is a standard non-algebraic account of why $1/2$
is special --- binomial sampling variance $\lambda(1-\lambda)$ is maximal
there \parencite{sakai2002edge} --- and it is not ours. What
...
\caption{The parity dichotomy across the census. Operators split by
cycle-parity class; the all-even class (which alone possesses fixed points,
Theorem~\ref{thm:parity}) is the one whose orbits preferentially die, and
the effect is monotone in the cycle count $k$.}
\label{fig:parity}
\end{figure}
...
of the algebra into a sustained dynamical object.

One composition is worth exhibiting. Taking the offset-$+2$
