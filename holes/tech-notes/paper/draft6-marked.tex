...
% ===================== REVISION MARKUP (draft6) =====================
% Marks claims affected by the 2026-07-27 tint-vs-LCS audit.
% Evidence: mmca-clj/holes/tint-vs-lcs-findings.md
\usepackage{soul}
\definecolor{markArtifact}{RGB}{255,148,148}
\definecolor{markInherit}{RGB}{255,226,130}
\definecolor{markWeak}{RGB}{178,214,255}
\newcommand{\artifact}[1]{{\sethlcolor{markArtifact}\hl{#1}}}
\newcommand{\inherit}[1]{{\sethlcolor{markInherit}\hl{#1}}}
\newcommand{\weakclaim}[1]{{\sethlcolor{markWeak}\hl{#1}}}
\newcommand{\flagged}[3]{%
  \par\vspace{3pt}\noindent
...
\hypersetup{
  hidelinks,
  pdftitle={Rule-Rewriting Cellular Automata and the Edges of Chaos},
...
\title{Rule-Rewriting Cellular Automata and the Edges of Chaos}
...
\artifact{against shuffled and mask-matched nulls, carry directed transport and
local combining that are both elevated in an intermediate band of a sampled
synthetic order--chaos family; under strong noise the transport ceases to be
specific to the filaments.} Temporal braids and a phenotype-reading
...
\begin{tcolorbox}[colback=white,colframe=black!55,title={\textbf{DRAFT 6 --- REVISION MARKUP, NOT FOR CIRCULATION}}]
\footnotesize
Claims affected by the tint-vs-LCS audit of 2026-07-27 are marked in place.
Evidence: \texttt{mmca-clj/holes/tint-vs-lcs-findings.md}; comparison in
\texttt{scripts/tint\_vs\_lcs\_compare.py}.
\smallskip

\noindent\artifact{RED --- known artifact.} The transport/combining measure is
local transfer entropy computed on the thresholded activity field
\mbox{$\mathrm{act}=(\mathrm{smooth}>0.35)$}, while the filament mask is the
\emph{gradient of that same field}. The mask therefore marks exactly the cells
where \mbox{$\mathrm{act}$} changes, and transfer entropy on a binary field is
near zero except where it changes (\mbox{$+0.8934$} on changed cells against
\mbox{$+0.0080$} elsewhere). Holding the mask at $0.35$ and moving only the
measure's threshold to $0.30$ collapses the filament-specific score from
\mbox{$+0.0885$} to \mbox{$+0.0007$}; with mask and measure matched the score
reappears at every threshold and grows monotonically with it (\mbox{$+0.065$} at
$0.25$ to \mbox{$+0.173$} at $0.50$). All three nulls displace the mask off the
change-cells, so none of them can detect this.
\smallskip

\noindent\inherit{AMBER --- inherits the compromised measure.} Comparisons
\emph{across} the sweep. Not void by construction, since an artifact of constant
form could still vary with $q$, but they must be re-derived under a measure that
does not share a threshold with the mask.
\smallskip

\noindent\weakclaim{BLUE --- underpowered or exploratory.} Already acknowledged
in the text; surfaced here so the revision pass sees them together.
\end{tcolorbox}
...
Recent computational experiments have shown that that accidentally
discovered operator is related to a family of operators that can be
given a concrete description, and whose emergent behaviour has
interesting properties.  A permutation $\sig$ of the eight truth-table
positions of an elementary rule defines an operator $P_\sig$ that
rewrites a rule by negating the bits $\sig$ names; there are $8! =
40{,}320$ such operators, and they admit exact classification rather
than sampling.  Asking this question at the level of this class of
operators rather than all possible ($8^8$) operators gives a more
manageable problem.

Asking it turns up two distinct ``edges of chaos''.  The first is the
classical one: a location in \emph{parameter space}, aligned with an
order parameter which can be tuned so that the dynamics sit between
order and disorder \parencite{langton1990computation}.  Our
classification lands on this critical point from an unexpected
direction---every fixed rule of every operator in the family is
balanced, with Langton activity $\lambda = 1/2$, exactly the value the
classical picture associates with the transition.  However, searching
the Meta Cellular Automaton (MetaCA) for a critical transition does
not produce one (Empirical finding~\ref{find:crossover}). The algebra
places the fixed points precisely at the classical critical value; the
dynamics find no critical point of this sort at all.

The second ``edge of chaos'' is not a location in parameter space at
all. Because the genotype layer varies from cell to cell, one lattice
can hold both ordered and disordered regions \emph{simultaneously}.
Tinting the genotype by activity resolves the field into coexisting
low- and high-activity domains, and the boundaries between those
domains trace fractal-like filamentary walls through spacetime.  We
observe that these walls are where the genotype is rewritten fastest,
\artifact{and---against a size-matched null, a geometry-preserving shift
null, and a cross-seed null---they carry directed transport that a
filament-shaped region placed anywhere else does not.}  This edge is
topographical: a feature of the picture rather than a critical point
for the parameter.

\artifact{Indeed, the spatial walls carry the most transport when the global
parameter sits in an intermediate band.  They lose their specificity when noise
dissolves the domains that define them.}  A parameter-space
region in which physical-space boundaries become computationally
active is a two-level claim, and neither level states it alone.

This paper presents a classification of the operators in question, a
census of the spatially coupled dynamics across all cycle types, and
the interface analysis that produced the second edge. The exact
results hold for the entire family. The empirical claims are based on
an examination of one synthetic order--chaos family, and one estimator
at history length one.
...
\node[con] (compute) at (river |- walls)
...
dynamical rather than a difference in rule composition. \weakclaim{The interface analysis is exploratory: it uses a handful of seeds
and a fixed-boundary variant,} and
...
taken up in \secref{sec:eoc-compute}.
\end{empiricalfinding}
...
and \emph{combined where they collide} \parencite{mitchell1993revisiting}. We
answer both within one synthetic family, and are explicit about what that buys
...
operator to $39\%$ under strong noise (Figure~\ref{fig:eoc-compute}c), at which
...
field. \artifact{We report the difference from that draw and call it
filament-specific transport or combining; it is zero when the filament is no
better than an arbitrary set of cells of the same size, and negative when it is
worse.}
\flagged{markArtifact}{ARTIFACT.}{The mask is the gradient of the same
thresholded field the transfer entropy is computed on, so a non-zero
filament-specific score is guaranteed by construction. See the banner on the
title page.}
...
\flagged{markArtifact}{ARTIFACT.}{Every filament-specific number in this
paragraph rests on the circular mask/measure construction and does not survive
decoupling the two thresholds.}
Within these bounds the bare operator's filaments transport. Over the eight
seeds of the $q=1$ sweep column, on-filament transfer entropy is
$+0.158 \pm 0.005$ bits against a shuffled surrogate of $+0.002$
($t = 33.5$, $p < 10^{-8}$) and a mask-matched draw of $+0.061$, leaving
$+0.097 \pm 0.005$ bits of filament-specific transport. On the single
fixed-boundary field of Figure~\ref{fig:interface} the same measure concentrates
$5.7\times$ over the domain interiors while its surrogate collapses to
$+0.0002$ bits, leaving $+0.135$ bits filament-specific; the
Rule~$110$-dominated $\sigma = 16250374$ field, as a positive control, shows the
same signature ($7.6\times$, surrogate $+0.001$, filament-specific $+0.161$
bits).

\begin{figure}[tb]
\centering
\begin{tikzpicture}
\node[inner sep=0.4pt, draw=black!25] (p) {\includegraphics[width=56mm]{figures/eoc_patch.pdf}};
...
$q=1,\,p=0$. Figure~\ref{fig:eoc-compute} reports both filament-specific
measures across this family.

\flagged{markInherit}{INHERITS THE MEASURE.}{A comparison \emph{across} $q$ at
fixed threshold, so not void by construction, but it inherits the compromised
measure and must be re-derived before it can be claimed.}
\begin{observation}[label={obs:eoc-compute}]{Transport and combining are elevated together in a transitional band}
Averaged over eight seeds per point, both filament-specific measures rise out of
the ordered regime, are elevated across an intermediate band of blend fractions
($q \approx 0.3$--$0.7$; transport $+0.155$ bits, combining $+0.041$ over the
band), and decay monotonically through the bare propagator into the noise-driven
regime (Spearman $\rho = -0.97$ and $-0.99$ over the chaotic flank, $n = 64$).
The band exceeds the bare operator on both measures ($t = 8.8$, $p < 10^{-8}$;
$t = 11.0$, $p < 10^{-7}$) and exceeds the ordered samples at $q \le 0.2$
($p < 0.001$). Its interior is not resolved: the largest single value falls at
$q = 0.4$ (transport $+0.172 \pm 0.011$ bits, combining
$+0.0444 \pm 0.0012$), but no other point in the band separates from it on
transport ($p \ge 0.058$), so we report a band and not a peak. Combining is thus
largest in a transitional regime---a literal edge of chaos in the sense of
\textcite{langton1990computation}---and is suppressed by both over-ordering and
disorder; the bare offset$+1$ operator sits below the band, on its descending
side.

The two measures part company under noise, and the split is the sharper result.
Filament-specific transport falls to zero and then \emph{inverts}: it is
positive at $p = 0.2$ ($+0.012$, $p < 10^{-5}$) and significantly negative from
$p = 0.4$ onward ($-0.005$, $p < 10^{-4}$; $-0.012$ at $p = 0.7$), meaning the
extracted wall carries less directed transport than an arbitrary set of cells of
the same size. Directed coupling itself does not vanish there---the raw score
still stands far above its shuffled surrogate---so noise does not destroy
transport in the field; it destroys the filament's ownership of it. Combining
remains weakly filament-specific throughout ($+0.0028$ at $p = 1$,
$p < 10^{-8}$).
\end{observation}

Two things bound this. The ascent into the band is not a density artefact:
across $q = 0.1$--$0.4$, where filament-specific transport grows $3.7$-fold, the
filament fraction is flat ($0.086$--$0.089$) and activity density rises, which is
the direction that would \emph{depress} the score given the sparse-limit
inflation above. The descent is weaker, because there the filament fraction
climbs monotonically alongside the decay and the two cannot be separated by
these data; what survives on that side is the sign change, which none of the
three nulls can manufacture. Beyond that, the sweep is one synthetic family
measured by one estimator at history length one, and a longer history or a
different decomposition of the joint contribution could move the numbers.

Within those bounds both halves of the question have an answer here. Information
is transported along the littoral filaments of these diagrams and combined on
them, most strongly at an intermediate regime that neither the ordered nor the
noise-driven extreme reaches; and where noise dissolves the walls, transport
stops being theirs to carry. What we cannot yet say is whether this holds beyond
the single family sampled here---that is the question for another paper.

\begin{figure}[tb]
\centering
\includegraphics[width=0.78\linewidth]{figures/edge_of_chaos_curve.pdf}
\caption{\textbf{A sampled edge of chaos.} Both measures across a synthetic
order--chaos family: the blend interrupter tunes toward order (fraction $q$), a
seeded stochastic rule-flip tunes toward chaos (rate $p$), with the bare
offset$+1$ propagator between them. \textbf{(a)}~\emph{transport} and
\textbf{(b)}~\emph{combining}, each plotted as the \emph{filament-specific}
score---the on-filament value minus a mask-matched random draw of the same cell
count---so that zero means the filament is no better than an arbitrary set of
cells that size. Points are means over $N=8$ seeds; bars are $\pm$SEM. Both are
elevated across the shaded transitional band ($q\approx0.3$--$0.7$) and decay
monotonically into chaos; on the chaotic flank transport inverts while
combining stays weakly positive. The open grey point at $q=0$ is bias-dominated
and is excluded from all claims. \textbf{(c)}~the filament fraction, which is
why the mask-matched null is needed: the extracted wall grows from $20\%$ to
$39\%$ of the field. Test statistics, the unresolved band interior, and the
single-family caveat are given in the text. Both nulls are seeded per field, so
the reported values are run-invariant; reproducible from
\texttt{mmca.experiments.eoc-sweep}.}
...
finding~\ref{find:river}) produces coherent bands that drift through a
...
thresholding a scalar activity tint, which is cheaper, coarser, and carries the
caveat recorded with it---the tint is a statistic of rule space, so equal-tint
neighbourhoods need not behave alike. The mapping above is therefore a reading
...
genotype--phenotype configuration, particles as their boundaries---would settle in a principled way what \secref{sec:eoc-compute} approaches
with a single-estimator proxy, and is the natural next instrument rather than
something attempted here.
\flagged{markArtifact}{UNDERSTATES.}{``Single-estimator proxy'' is now too mild.
The measure and the mask are derived from the same thresholded field, so this is
a circularity rather than a question of estimator choice. A first attempt at the
principled treatment was run (6 seeds, W=64, T=120): it agrees with the tint at
chance level ($\kappa = 0.008$) and shows no filament-specific transport
($+0.004$ against the size-matched null). That reconstruction selected depth and
tolerance at the edge of its search grid, so it is not yet decisive on its own.}
...
entropy-rate statistics. The information measures of
\secref{sec:eoc-compute} are reported against three stated nulls---one
size-matched, two geometry-preserving, in agreement---and within a stated
density range, for one synthetic family at one history length; the local
transfer entropy estimator is shown there to be unusable on near-degenerate
activity fields,
and we make no cross-operator claim from it.
