...
neighbourhoods defines an operator $P_\sig$ on those rules by
$\bit{\sig(k)} := \lnot\,\bit{k}$. The resulting family contains
$8! = 40{,}320$ permutation-propagators and is exhaustively classifiable.

The exact, machine-checked classification is that $P_\sig$ has a fixed rule if
and only if every cycle of $\sig$ is even; an all-even permutation with $k$
cycles has $2^k$ fixed rules; and every fixed rule is balanced, with Langton
activity $\lambda=1/2$. A census of the spatially coupled dynamics finds that
every odd-cycle operator is genotype-alive under the stated finite-run
protocol, whereas every all-even cycle type contains both sustaining and
collapsing operators. All four genotype/phenotype activity regimes occur among
the bijective operators.

Within the wrap-rotation subfamily under neighbour-blend coupling, the
coprime rotations sustain a high-diversity field while the other nontrivial
rotations collapse in the runs tested. A finite-size scan finds no evidence of
a sharpening critical point over the sizes studied, but instead a broad,
drifting crossover---a finite-width \emph{littoral edge}. Activity-tinted fields
contain filamentary boundaries on which genotype rewriting is elevated. Finally,
time-dependent braids of propagators and a phenotype-reading construction step
provide two reproducible ways to sustain structure beyond the bare operator.}
...
that rule field alongside the phenotype. An elementary cellular automaton
...
truth-table positions. Its elementary update at source $k$ is
\[
(U_{w,k}g)(j)=
\begin{cases}
\lnot g(k),&j=w(k),\\
...
\end{cases}
\]
Thus one update complements source bit $k$ and writes it to target $w(k)$. If
...
bytes are exactly the bytes fixed by every $U_{\sig,k}$. For a non-injective
writing, individual updates $U_{w,k}$ remain well defined, but a simultaneous
...
the rotation survey, finite-size scan, and interface analysis. The constructive
branch (green) tests two ways of sustaining structure. Arrows mark logical or
methodological dependence; figures visualise the corresponding measurements but
are not themselves premises.}
\label{fig:map}
\end{figure}
...
\]
\item \textbf{Write one propagated bit.} Draw a source
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{figures/fig2pair.png}
...
\begin{figure}[p]
\centering
\includegraphics[height=0.73\textheight]{figures/survey_a.png}
...
\ContinuedFloat
\centering
\includegraphics[height=0.73\textheight]{figures/survey_b.png}
...
alone are \emph{braided} by alternating their elementary spatial updates. In the
tested contrast, the braid sustains when the operators preserve \emph{different}
block systems. Top: offset$+2$ (two
...
partition, so the alternation never leaves it. The result concerns the
time-dependent coupled dynamics, not composition of the simultaneous maps
$P_\sigma$.}
\label{fig:braid}
\end{figure}
...
\caption{\textbf{A diversity-control schedule.} The powers of an 8-cycle
...
(low) to a high-diversity field as the schedule changes---the same two operators
throughout, with only their relative frequency varied.}
\label{fig:knob}
\end{figure}
...
Rule~110 is, if anything, depleted), so the distinction is dynamical rather than
a difference in rule composition. The interface analysis is exploratory: it
uses a handful of seeds and a fixed-boundary variant, and the denser river
boundary ($D\approx1.7$ versus $1.5$) is an unmatched comparison that does not
establish an effect of genotype--phenotype coupling. Whether rewriting transports
or combines information \emph{along} the walls is not tested here.
\end{empiricalfinding}
...
range.} Offset $+2$ feedforward finite-size scan against the
...
\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figures/eoc_tint.png}
...
the two-4-cycle $[6\,7\,0\,2\,1\,4\,3\,5]$---the standard-order form of draft3's
offset$+2$ exemplar---forms \emph{large coherent} intermediate-activity domains
