...
\begin{expositionbox}{Balance is exact; edge-of-chaos is a separate question}
Our main result concerns where those fixed points \emph{are}: without exception,
every fixed point has exactly four one-bits---a \emph{balanced} table, Langton's
$\lambda = 1/2$---forced by parity. A balance long tied \emph{empirically} to the
edge of chaos is here an exact, elementary consequence of the fixed-point
condition. Whether it \emph{behaves} as an edge of chaos the family answers only
\emph{sometimes}: $\lambda = 1/2$ is shared by operators that collapse and by the
8-cycles that do not, so $\lambda$ alone cannot separate a diverse field from a
collapsed one. For that we use a simple population observable---the count of
distinct rules over time.
\end{expositionbox}
