...
\tcolorboxenvironment{definition}{%
  ...
  boxrule=0.7pt, arc=1.5mm, left=2.5mm, right=2.5mm, top=1.5mm, bottom=1.5mm}
% Titled exposition box; break a long one into one-point sub-boxes (\pointbox).
\newtcolorbox{expositionbox}[2][]{%
  enhanced, breakable, colback=gray!4, colframe=gray!55!black,
  boxrule=0.6pt, arc=1.2mm, left=2.5mm, right=2.5mm, top=1.5mm, bottom=1.5mm,
...
More generally, a \emph{writing} is any function
$w\colon\{0,\dots,7\}\to\{0,\dots,7\}$ on the truth-table positions, and its
\emph{propagator} complements each source bit and writes it to the target
$w(k)$, namely $\bit{w(k)} := \lnot\,\bit{k}$. When $w$ is a bijection---a
permutation $\sig\in S_8$---each target is written exactly once, $\sig^{-1}$
exists, and the propagator takes the closed deterministic form~\eqref{eq:prop};
these \emph{permutation-propagators} are the object studied throughout. A general
writing need not be injective: two sources may share a target (a
\emph{collision}, which makes the simultaneous rewrite order-dependent) and some
position may be written by none (a \emph{gap}). Permutations are only the
bijective core of a far larger class---there are $8!=40{,}320$ permutations but
$8^{8}=16{,}777{,}216$ writings, so the permutations are $0.24\%$ of the whole.
The cycle-structure algebra of \secref{sec:classification} is available only on
this core; the historical operator of Figure~\ref{fig:figure8},
$[0\,0\,1\,2\,3\,4\,5\,6]$, is itself a non-bijective writing (bits $0$ and $1$
both write to position $0$, and position $7$ is never written), and
Figure~\ref{fig:fig2pair} shows how far its behaviour ranges between its two
...
number of cycles $k$. We first report this as an observed trend: the present
spatial runs fix a single lattice size, boundary condition, initial
distribution, and horizon. The full permutation census
(\secref{sec:full-census}) then supplies the by-class tabulation for genotype
collapse over all $40{,}320$ operators, seed-averaged---turning the trend into
the measured law of Table~\ref{tab:census}. Note also that classifying the enumerated
...
$40{,}320$ permutation-propagators---three seeds each, scoring genotype and
phenotype aliveness by the late-window change rate ($\ge 0.10$)---and grouped
them by cycle type (Table~\ref{tab:census}).
