...
$\lambda = 1/2$ is shared by operators that collapse and by the 8-cycles
...
\centering
\begin{tikzpicture}[
  font=\footnotesize, node distance=6mm and 10mm,
  bx/.style={draw, rounded corners=1.5pt, align=center, inner sep=3pt,
  ...
  we/.style={bx, fill=green!10, draw=green!45!black, text width=27mm},
  ef/.style={bx, fill=orange!12, draw=orange!65!black},
  cen/.style={bx, fill=gray!12, draw=gray!55!black},
  supp/.style={-{Stealth[length=1.7mm]}, draw=black!65, semithick},
  illu/.style={-{Stealth[length=1.5mm]}, draw=gray!55, dashed},
]
\node[defn] (def) {\textbf{Def.\,1}\\Writing \& permutation};
\node[thm, below=11mm of def] (t1) {\textbf{Thm\,1}\\Fixed point $\iff$ every cycle even};
\node[thm, below=of t1] (t2) {\textbf{Thm\,2}\\Every fixed point balanced, $\lambda=\tfrac12$};
\node[obs, right=of def] (obs) {\textbf{Obs.\,1}\\Permutations are $0.24\%$ of all writings};
\node[ef, right=of t1] (ef) {\textbf{Emp.\,find.\,1}\\Survey: 8-cycles sustain, offset\,$\pm2,\pm4$ collapse};
\node[cen, right=of t2] (cen) {\textbf{Census}\\Collapse rate by cycle type (Table\,\ref{tab:census})};
\node[we, left=of def] (we12) {\textbf{WE\,1--2}\\A propagator, one MetaCA step, by hand};
\node[we, left=of t1] (we3) {\textbf{WE\,3}\\Even \& odd cycles};
\node[we, right=of ef] (we4) {\textbf{WE\,4}\\Reading the survey};
\draw[supp] (def) -- node[right,font=\scriptsize,pos=0.4]{restrict} (t1);
\draw[supp] (t1) -- (t2);
\draw[supp] (def) -- (obs);
\draw[supp] (t1) -- node[above,font=\scriptsize]{dynamics} (ef);
\draw[supp] (t1.south east) to[out=-20,in=180] node[below,font=\scriptsize,pos=0.7]{frequency} (cen.west);
\draw[supp] (ef) -- (cen);
\draw[illu] (we12) -- (def);
\draw[illu] (we3) -- (t1);
\draw[illu] (we4) -- (ef);
...
dashed arrows mark a worked example illustrating a box. Colour encodes type:
definition (violet), theorem (blue), observation (teal), empirical finding
(amber), worked example (green), census (grey). The permutation definition
restricts to the classified core; Theorem~\ref{thm:parity} (fixed-point
existence) grounds both the empirical survey and the census that measures how
often collapse occurs; Theorem~\ref{thm:lambda} pins where the fixed points sit.
Everything load-bearing is a box; the connecting prose is scaffolding.}
\label{fig:map}
\end{figure}
...
(offset $\pm 1$, $\pm 3$) sustain a high-diversity, visually complex phase, while
offset $\pm 2$ and $\pm 4$ collapse to near-uniform fields. That surviving phase is visually complex, and a reader is entitled to ask whether
