...
\begin{tikzpicture}[font=\footnotesize, >={Stealth[length=1.6mm]},
  bx/.style={draw, rounded corners=1.5pt, align=center, inner sep=2.5pt,
             text width=28mm, minimum height=7.5mm},
...
\node[defn] (dgp) at (0,-1.55) {\textbf{Def 2} Genotype \& phenotype};
\node[defn] (dlam)at (0,-3.1)  {\textbf{Def 3} Activity $\lambda$};
\node[defn] (dint)at (0,-4.65) {\textbf{Def 4} Interrupter};
\node[defn] (dal) at (0,-6.2)  {\textbf{Def 5} Aliveness};
\node[obs]  (obs) at (4.6,0)     {\textbf{Obs 1} Permutations $=0.24\%$};
\node[thm]  (t1)  at (4.6,-2.3)  {\textbf{Thm 1} Fixed pt $\Leftrightarrow$ all cycles even};
\node[thm]  (t2)  at (4.6,-4.4)  {\textbf{Thm 2} Fixed pts balanced, $\lambda=\tfrac12$};
\node[defn] (driv)at (4.6,-6.2)  {\textbf{Def 6} River};
\node[ef]   (ef)  at (9.2,-2.3)  {\textbf{Emp.\ find.\ 1} 8-cycles sustain};
\node[cen]  (cen) at (9.2,-4.4)  {\textbf{Census} collapse by cycle type};
\draw[s] (d1)--(obs);
\draw[s] (d1)-- node[midway,above,font=\scriptsize]{restrict} (t1);
\draw[s] (dgp)--(t1);
\draw[s] (dlam)--(t2);
\draw[s] (t1)--(t2);
\draw[s] (t1)-- node[midway,above,font=\scriptsize]{dynamics} (ef);
\draw[s] (t1)-- node[midway,above,font=\scriptsize,pos=0.75]{frequency} (cen);
\draw[s] (dal.east) to[out=0,in=205] (cen.west);
\draw[s] (dint)--(driv);
\draw[s] (ef)--(cen);
\end{tikzpicture}
\caption{\textbf{Map of the paper by box.} Every load-bearing claim is a box;
arrows carry logical support and colour encodes type---definition (violet),
theorem (blue), observation (teal), empirical finding (amber), census (grey).
The object and its measurements (Definitions~1--5) support the two theorems, the
empirical survey, and the census: Theorem~\ref{thm:parity} (fixed-point
existence) grounds both the survey and the census that measures how often
collapse occurs, while Theorem~\ref{thm:lambda} pins where the fixed points sit;
the interrupter (Def.~4) yields the river (Def.~6). Worked examples~1--4
illustrate the boxed claims. The connecting prose is scaffolding: anything not
boxed is expository.}
\label{fig:map}
\end{figure}
