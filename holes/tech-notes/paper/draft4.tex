...
\begin{empiricalfinding}[label={find:crossover}]{No critical point in the crossover; growing fluctuations at the pure-propagator limit}
The natural control parameter is the \emph{propagator fraction} $q$ ($q = 0$ all
blend, $q = 1$ all propagator). Across widths $L = 30$--$240$ (32 seeds each) the
genotype activity $a_G(q)$ rises \emph{smoothly}, well described by a logistic
crossover ($R^2 > 0.99$, Figure~\ref{fig:phase}a); crucially its width does not
shrink with size ($w \approx 0.13$, $w \sim L^{-0.04}$), so the curve never
steepens into the step a genuine critical transition would ($w \sim L^{-1/\nu}
\to 0$). A second test---the size-scaled run-to-run spread $L\,\mathrm{Var}(a_G)$
(Figure~\ref{fig:phase}b)---stays flat through the band (self-averaging) but
\emph{grows} at $q = 1$, the pure-propagator limit at which offset $+2$ runs: the
signature of activity organised into \emph{system-spanning domains}
(Figure~\ref{fig:tint}). There is thus no critical point within the band; the
fluctuations concentrate at $q = 1$, where the coexisting domains, walls, and
genotype churn live. As $q = 1$ is the scan's boundary we claim no confirmed
critical point there---only growing, non-self-averaging fluctuations.
\end{empiricalfinding}
