...
spatial dynamics, the parity dichotomy also shows in behaviour---but as a
tendency, not a law. Only the all-even operators \emph{can} settle onto a fixed
rule, and some do (rotate$+2$), while others stay diverse though all-even
(rotate$+1$); the any-odd operators, having no fixed rule, never settle
(Figure~\ref{fig:parity}). The full census below (\secref{sec:full-census}) makes
this quantitative over all $40{,}320$ operators (Table~\ref{tab:census}): the
dynamical content is the \emph{death-rate difference} between the classes, not a
clean collapse of the all-even class.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{../../labs/M-aif-tokamak/compositions/parity-dichotomy.png}
\caption{The parity dichotomy by example, illustrating Theorem~\ref{thm:parity}
(a fixed rule exists iff every cycle is even). Five operators run alone, as
genotype (top) and phenotype (bottom). Both \emph{all-even} operators possess
fixed rules---but existence is not attraction: rotate$+2$ (cycles $4\,4$)
\emph{settles} onto its $2^2 = 4$ fixed rules (diversity $4$, a regular
phenotype), whereas rotate$+1$ (a single $8$-cycle) does \emph{not} settle---it
stays diverse (diversity $12$, an active phenotype), all-even yet alive. The
three \emph{any-odd} reduced operators possess no fixed rule at all, cannot
settle, and stay richly diverse (diversity $27$--$56$). That rotate$+1$ and
rotate$+2$ share their parity but not their fate is the census in miniature
(Table~\ref{tab:census}): all-even makes collapse \emph{possible}, not
inevitable.}
\label{fig:parity}
\end{figure}
...
$9.8\%$ live/dead, $6.0\%$ dead/dead, $1.4\%$ dead/live---so even
Figure~\ref{fig:fig2pair}'s live/dead mirror is reached by $3{,}953$
\emph{permutations}, not only by the
...
tool: complementary dead operators synthesise a sustained, high-diversity braid.}
\label{fig:braid}
\end{figure}
...
(low) to a rich, high-diversity field (high) as the knob is turned---the same two
operators throughout, only the schedule changing. Composition is thus not just constructive
...
$D \approx 1.5$--$1.8$ over $L = 128$--$768$, a handful of seeds). Those walls are
...
$L = 256$. All the sustaining operators carry fractal-like boundaries
($D \approx 1.5$--$1.8$), but of visibly different density: offset $+1$ (the
8-cycle) and $\sigma = 16250374$ carry dense walls ($D \approx 1.6$), the
phenotype-coupled river the densest ($D \approx 1.80$, an unmatched comparison as
it couples phenotype), while the two-4-cycle $[6\,7\,0\,2\,1\,4\,3\,5]$---the
standard-order form of draft3's offset$+2$ exemplar---carries \emph{sparse, clean}
walls bounding large single-activity regions ($D \approx 1.53$), matching its
coherent domains in Figure~\ref{fig:tint}. The dimension is stable across
$L = 128$--$768$ (box sizes $2$--$64$); collapsed operators (e.g.\ offset $+4$)
carry none. These walls are also where the rule field is rewritten fastest
(Emp.~find.~\ref{find:walls}). Exploratory: a handful of seeds, fixed-boundary
variant.}
\label{fig:interface}
\end{figure}
