...
system runs. An operator is defined by a permutation $\sig$ of the eight truth-table
...
we find no critical point. We measure instead how far the effect of a single-cell
...
rate lowers it. Finally, individual cells cannot choose between operators from
local observation: the relevant local field decorrelates in about six steps, less
than the observation window that discrimination requires, and copying rules across
phase boundaries accelerates the freezing it was intended to prevent. The
coexisting state itself has a strictly local specification---the lattice that
produced it ran one---but no locally computable quantity we tested identifies
that outcome from within a run, and the local interventions that do prevent
freezing hold a foam of small frozen islands, not the found configuration's
extended regions.}  \keywords{cellular automata, rule evolution,
...
\pstep{1.1.1}{\tagdef\ An operator is a permutation $\sig$ of the eight
