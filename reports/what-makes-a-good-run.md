...
This report replays eight historically significant runs through the
TPG diagnostic pipeline and reads their signatures. It includes both
MMCA-runtime champions and a wiring-ladder standout (L5-creative) that
exhibits a qualitatively different kind of structure. Then it contrasts
those signatures against runs generated by the new TPG controller, to
identify what the TPG needs to learn.
...
## 3. The Genotype Outlier: L5-Creative

### Seed 352362012: Boundary-Guardian Wiring (100 cells, 100 generations)

![l5-creative-352362012](../out/best-run-characterizations/l5-creative-352362012.png)

*L5-creative: boundary-guardian gate selects legacy kernel (stable) or XOR-creative
path based on local diversity > 0.5. Time flows downward.*

This run comes from the wiring ladder experiment, not the MMCA runtime —
it uses a different execution path (`wiring/runtime`) and a fundamentally
different control structure. The boundary-guardian wiring gate checks each
cell's local neighborhood diversity and routes it to either a conservative
legacy kernel or a creative XOR path. The result is a run where the
**genotype itself** shows edge-of-chaos dynamics.

```
Diagnostic signature:
  H  entropy             ██████████████████░░ 0.946 ± 0.011
  Δ  change              ███████████████████░ 0.985 ± 0.099
  ρ  autocorrelation     ░░░░░░░░░░░░░░░░░░░░ 0.015 ± 0.099
  σ  diversity           ████████████████░░░░ 0.828 ± 0.031
  φ  phenotype-coupling  ██░░░░░░░░░░░░░░░░░░ 0.149 ± 0.002
  λ  damage-spread       ███████████████████░ 0.990 ± 0.070

  Tai-zone satisfaction: 0.176
```

The tai-zone satisfaction is low (0.176) because the standard verifier bands
are calibrated for the MMCA champion regime — moderate change, moderate
autocorrelation. L5-creative operates in a completely different regime:
nearly **100% genotype change rate** (0.9952 avg), near-zero autocorrelation,
and very high diversity. By standard metrics, it looks chaotic.

But visual inspection tells a different story. The spacetime diagram shows
diagonal coherence and "stunted tree" patterns reminiscent of Rule 90 —
structure that lives in the **genotype bitplanes** rather than in the
symbol-level statistics. Prior analysis found:

```
Genotype bitplane analysis:
  Best bitplane:          5
  Triangle clearings:     40
  Triangle score:         0.133
  Diagonal autocorr:      0.015 (genotype), 0.502 (phenotype)
  Structured-chaos score: 0.168 (phenotype layer)

Phenotype dynamics:
  Change rate:            0.500 (balanced, not chaotic)
  Frozen columns:         0%
  White/black ratio:      0.44 / 0.56
```

**Why it matters:** In every MMCA champion run, the genotype is the
structured layer and the phenotype follows. L5-creative inverts the
usual relationship — the phenotype is well-behaved (change ~0.5, no
barcode) while the genotype churns at nearly 100% turnover. Yet that
churning genotype carries *spatial* structure visible in bitplane
decomposition: triangle clearings, diagonal streaks, regions where
the boundary-guardian gate persistently selects one path over the other.

This is the only run in our collection where the genotype layer exhibits
edge-of-chaos dynamics. Most runs show either frozen genotype (Class I/II)
or uniformly chaotic genotype (Class III). L5-creative achieves Class IV-like
structure in the genotype — a qualitatively different achievement that the
current 6D diagnostic vector doesn't fully capture.

**Implication for TPG:** The standard diagnostic pipeline sees L5-creative
as chaotic because it measures symbol-level change rates. A richer diagnostic
— perhaps incorporating bitplane entropy, diagonal autocorrelation, or
triangle density — would distinguish L5-creative's structured chaos from
true randomness. This is a gap the TPG verifier system should eventually
address.

![l0-baseline-352362012](../out/best-run-characterizations/l0-baseline-352362012.png)

*L0 baseline (same seed): pure legacy kernel, no creative path. Genotype is
completely frozen (change rate 0.000) — the opposite extreme.*

---

## 4. The Shorter Champions
...
## 5. What Good Runs Have in Common

The seven MMCA-runtime runs cluster in a tight region of diagnostic space.
L5-creative (wiring-ladder) occupies a separate region entirely:

```
                    Champion  Baseline  Exotic工   M07A    M07B     丁      Noop    L5-creat
                    ────────  ────────  ────────  ──────  ──────  ──────  ──────  ────────
H  entropy           0.863     0.849     0.849    0.831   0.881   0.854   0.854    0.946
Δ  change            0.310     0.312     0.314    0.303   0.314   0.291   0.305    0.985
ρ  autocorrelation   0.690     0.688     0.686    0.697   0.686   0.709   0.695    0.015
σ  diversity         0.602     0.574     0.573    0.626   0.704   0.672   0.680    0.828
φ  phe-coupling      0.141     0.142     0.144    0.173   0.177   0.195   0.197    0.149
λ  damage-spread     0.675     0.682     0.695    0.641   0.662   0.641   0.687    0.990
                    ────────  ────────  ────────  ──────  ──────  ──────  ──────  ────────
Tai-zone sat.        0.992     0.996     0.996    0.911   0.815   0.879   0.871    0.176
```

L5-creative is an outlier on every dimension except entropy and phenotype-coupling.
It belongs to a different dynamical regime — one that the current verifier doesn't
reward but that produces visually striking genotype-layer structure.
...
## 6. The Contrast: TPG-Generated Runs
...
## 8. What This Means for TPG Evolution
...
### L5-creative reveals a diagnostic blind spot

The current 6D diagnostic vector cannot distinguish L5-creative's structured
genotype chaos from true randomness — both show Δ≈1.0, ρ≈0.0. Yet the
spacetime diagram clearly shows spatial organization. Candidate additional
diagnostics:

- **Bitplane entropy**: decompose each sigil into bit-planes and measure
  entropy per plane — structured chaos has lower entropy on specific planes
- **Diagonal autocorrelation**: measure correlation along the (x+1, t+1)
  diagonal, not just along the time axis — diagonal streaks show up here
- **Triangle density**: count Rule-90-style triangle clearings in bitplanes

Adding even one of these would let the verifier reward L5-creative-style
runs, opening a second "good regime" beyond the MMCA champion signature.
...
6. **Bitplane diagnostics for L5-creative regime.** Implement diagonal
   autocorrelation and/or triangle density as additional diagnostic
   dimensions. Test whether these new dimensions can distinguish
   L5-creative's structured genotype chaos from true Class III chaos,
   enabling a second verifier spec for the genotype-EoC regime.
...
| File | Description |
|------|-------------|
| `scripts/characterize_best_runs.clj` | Replay script: runs 7 MMCA champions through diagnostic pipeline |
| `scripts/characterize_l5_creative.clj` | Replay script: L5-creative boundary-guardian characterization |
