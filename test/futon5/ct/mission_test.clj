...
;;; ============================================================
;;; Parallel Composition (Shared Constraints)
;;; ============================================================
...
(deftest test-compose-parallel-social-coordination
  (testing "compose-parallel merges shared constraint inputs and validates"
    (let [social (read-mission "data/missions/social-exotype.edn")
          coordination (read-mission "data/missions/coordination-exotype.edn")
          composed (mission/compose-parallel social coordination)]
      (is (some? composed) "parallel composition should succeed")

      ;; Ratchet: social exotype uses :social timescale for its 7 components.
      (is (= 7 (count (:components social))))
      (is (every? #(= :social (:timescale %)) (:components social))
          "social components should be :social timescale")

      ;; Shared constraint ports are unprefixed and deduped.
      (let [inputs (get-in composed [:ports :input])]
        (is (= 1 (count (filter #(= :I-patterns (:id %)) inputs))))
        (is (= 1 (count (filter #(= :I-registry (:id %)) inputs)))))

      ;; Edges referencing shared constraints point to the merged ids.
      (is (some #(and (= :I-patterns (:from %))
                      (= (kw* :coordination-exotype :G3) (:to %)))
                (:edges composed))
          "coordination edge from I-patterns should point to a prefixed component")
      (is (some #(and (= :I-patterns (:from %))
                      (= (kw* :social-exotype :S-model) (:to %)))
                (:edges composed))
          "social edge from I-patterns should point to a prefixed component")

      ;; Cross-diagram connection: task submissions feed coordination request input.
      (is (some #(and (= (kw* :social-exotype :O-task-submissions) (:from %))
                      (= (kw* :coordination-exotype :I-request) (:to %))
                      (= :http-request (:type %)))
                (:edges composed))
          "expected O-task-submissions -> I-request edge")
