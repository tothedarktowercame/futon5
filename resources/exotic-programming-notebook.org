...
** Mission 7 (draft): Exotic programming layer

- Goal: make exotypes first-class programs by defining a compositional CT category and a manifest for the 256 super-sigils.
- Approach: introduce a new category (`:futon5/exotype-program`) with labeled inputs/outputs, then map each sigil to a program template + params.

*** Positioning: viability vs optimization
- Goal-optimization (e.g., locking Rule 110) is trivial but misses the point: we want lifelike regimes, not a fixed target.
- Exotic programming selects evaluator programs that keep evolution productive; xenotypes police viability, not target attainment.
- The relevant question is whether evaluators preserve open-ended, discriminative dynamics across time and seeds.

*** Generalized Baldwin effects (interface view)
- Exo ↔ Geno: exotypes shape kernel mutation/selection pressures, so short-horizon evaluation acts as a Baldwin-like scaffold for genotypes.
- Xeno ↔ Exo: xenotypes shape evaluator evolution, so long-horizon viability acts as a Baldwin-like scaffold for exotypes.
- Together this yields a coherent narrative: each layer supplies a “learned” bias that becomes selection pressure for the layer below, avoiding ad hoc tuning.

*** Category sketch
- Objects: `:history`, `:context`, `:kernel-spec`, `:params`, `:rng`
- Morphisms: `:sample-context`, `:gate-update`, `:contextual-mutate`, `:set-mix`, `:normalize`
- Context step: `[:history :rng] -> :context` (history = genotype + phenotype traces)
- Kernel transformer: `[:kernel-spec :context :params :rng] -> :kernel-spec`
- Semantics: stochastic/Kleisli (morphisms are probabilistic transformations conditioned on `:rng`)

*** Composition conventions
- Composition order is left-to-right in the manifest (the `:parts` list is the evaluation order).
- This is explicit in `:composition-order :left-to-right` to avoid CT notation ambiguity.

*** Generate manifest (reproducible)
#+BEGIN_SRC bash
bb -cp src:resources -e '(require (quote [futon5.ct.exotype-programs :as xp]))
(spit "resources/exotype-program-manifest.edn" (pr-str (xp/manifest :super)))'
#+END_SRC

Artifacts:
- resources/exotype-program-manifest.edn

*** Notes
- The manifest encodes the program template plus sigil-derived params for each super-sigil.
- Word variation is currently by tier (local vs super); sigils vary params, not the morphism word.
- Mapping back to `:futon5/meta-kernel` is deferred; this keeps the exotype-program category explicit and compositional.

*** Example: super-sigil 11110100 (仪)
#+BEGIN_SRC edn
{:sigil "仪"
 :bits "11110100"
 :tier :super
 ...
 :program {:template :contextual-mutate+mix
           ...
           :diagram {:type :compose
                     :data {:parts [{:data {:name :gate-update}}
                                    {:data {:name :contextual-mutate}}
                                    {:data {:name :set-mix}}
                                    {:data {:name :normalize}}]}}
           :context-step {:type :primitive
                          :data {:name :sample-context}}}
 :params {:rotation 3
...
Interpretation:
- Objects/morphisms: `:history` samples a `:context`, then the kernel transformer composes `:gate-update -> :contextual-mutate -> :set-mix -> :normalize`.
- Compiled evaluator shape: `sample-context` selects a local (pred/self/next/out + phenotype bits), `gate-update` probabilistically mutates, `set-mix` applies super-tier mixing, and `normalize` clamps the spec.

*** Protocol (draft)
Regimes:
- Regime A (direct optimization): evolve genotypes with a fixed proxy fitness (e.g., composite score).
- Regime B (flat exotypes): evolve exotypes as 8-bit sigils (params only) under xenotype guard; genotypes evolve under those evaluators.
- Regime C (program words + params): keep the 8-bit sigil as an address, but allow the program word to mutate via small rewrites, while params mutate as before.

Allowed rewrites (word-level):
- Insert/remove `gate-update` (respecting invariants).
- Swap `set-mix` variants (or toggle on/off in super tier).
- Move `normalize` within allowed invariants (must remain terminal for kernel validity).
Rewrite operators can be deferred to a follow-on ticket if we want to run Regime C later.

Lock modes (nomination -> locked rerun):
- Lock-0 (kernel only): lock initial kernel-spec; rerun evaluator with fresh rng.
- Lock-1 (kernel + rng seed): lock kernel and rng seed; rerun exactly.
- Lock-2 (kernel + context schedule): lock kernel and sampled contexts; rerun with identical experience.
Pick one lock mode per analysis and report it explicitly (generalization and nomination gap are lock-mode dependent).
Pilot note: run all three lock modes in the pilot to compare sensitivity; choose one for the main sweep based on signal.

Selection logic (xenotype as constraints + portfolio):
- Step 1 (cull): discard exotypes that violate non-degeneracy thresholds (always-pass/fail, early stasis, confetti, short cycles).
- Step 2 (rank): among survivors, rank by discriminative signal and nomination -> locked rerun stability.
- Step 3 (portfolio): keep top-k per word class to preserve evaluator diversity.

Measurements (xenotype duties mapped to failure modes):
- Non-degeneracy: dead/confetti rates, early stasis, short-cycle detection.
- Discriminative signal: score variance across genotypes/exotypes, diversity over time.
- Open-ended dynamics: long transients, metastable bands, lesion recovery.
- Generalization: nomination vs locked rerun gap across seeds (by lock mode).

Structure-aware analysis (CT earns its keep):
- Record the exotype program word (sequence of primitives) and invariants per evaluator.
- Report collapse rates, generalization gaps, and discriminative variance conditioned on word class.

Context-only baseline (guard against context hacks):
- Randomize or mask genotype contributions and re-run the evaluator.
- If discrimination remains strong, treat as a context/phenotype shortcut and cull/penalize at the xenotype layer.

*** Pilot (A/B) plan
Scope (small but representative):
- Length 50, generations 30 (match Mission 4/6 defaults).
- Runs 200, population 16, update cadence 100, seed 4242.
- Regime A + B both use the same seeds and measurement hooks.

Regime A (direct optimization, pilot):
- Use the genotype optimizer (score by composite metric, mutate, select).
- Example command (pilot-sized):
#+BEGIN_SRC bash
bb -cp src:resources -m futon5.mmca.genoevolve \
  ...
  --mutation-rate 0.1 \
  --seed 4242 \
  --log /tmp/mission-07-a.edn
#+END_SRC

Regime B (flat exotypes, pilot):
- Reuse the Mission 6 xenotype spec and exoevolve runner.
- Example command (pilot-sized):
#+BEGIN_SRC bash
...
bb -cp src:resources -m futon5.mmca.exoevolve \
  ...
  --xeno-weight 0.5 \
  --log /tmp/mission-07-b.edn
#+END_SRC

Helper: lock-mode reruns (Lock-0/1/2, nomination mode)
#+BEGIN_SRC clojure
(require '[futon5.mmca.runtime :as mmca])

(defn lock-mode-reruns
  [{:keys [genotype phenotype exotype generations kernel seed]}]
  (let [seed (long (or seed 4242))
        base {:genotype genotype
              ...
              :exotype exotype
              :exotype-mode :nominate}
        nomination (mmca/run-mmca (assoc base :seed seed
        ...
        lock2 (mmca/run-mmca (assoc base :seed seed
                                         :exotype-contexts ctxs))]
...
Notes:
- The lock-mode helper runs in `:exotype-mode :nominate` so the kernel stays fixed while the evaluator is exercised.
- Lock-2 replays the exact context schedule via `:exotype-contexts`; use it to measure evaluator sensitivity to sampling noise.

Lock-mode comparisons (pilot only):
- For each regime, pick top-N exemplars (N=5) and run Lock-0/1/2 reruns side-by-side.
- Lock-2 uses `:capture-exotype-contexts true` on nomination, then replays via `:exotype-contexts` on the rerun.

Outputs:
- EDN logs for A and B (e.g., /tmp/mission-07-a.edn, /tmp/mission-07-b.edn).
- CA printouts for nomination + lock reruns, with lock-mode labels in filenames.

*** Pilot quick look (A/B snapshots)
- Regime A top scores: 45.93, 43.10.
- Regime B top scores: 50.41, 49.59 (both sigil 兄, bits 11100100, super tier).
- Regime B snapshots reseed genotype/phenotype from the run seed (exoevolve logs do not store them yet).

*** Pilot summary (A vs B)
...
| q90 | 38.57 | 46.62 |
| max | 45.93 | 50.41 |
| confetti-rate | 0.04 | 0.065 |
| dead-rate | 0.00 | 0.00 |
| stasis-mean | 23.67 | 20.82 |

Pilot interpretation:
- Regime B shifts the score distribution upward and reduces stasis time on average, but carries a modest confetti increase.
- Lock-2 preserves nomination structure best, indicating evaluator context sampling is a real sensitivity axis.
- Context-only baselines damp structured bands while keeping exotype stripes active, supporting a xenotype guardrail against context/phenotype shortcuts.
...
(defn write-meta! [path m]
  (spit path (pr-str m)))

(defn run-image-a! [idx entry]
  (let [label (format "mission-07-a-top%d" idx)
        ppm-path (str out-dir "/" label ".ppm")
        meta-path (str out-dir "/" label ".edn")
        result (mmca/run-mmca {:genotype (:genotype entry)
                               :phenotype (:phenotype entry)
                               :generations (:generations entry)
                               :kernel :mutating-template
                               :operators []
                               :seed (:seed entry)})
        summary (metrics/summarize-run result)]
    (render/render-run->file! result ppm-path)
    (write-meta! meta-path {:label label
                            :seed (:seed entry)
                            :length (:length entry)
                            :generations (:generations entry)
                            :kernel :mutating-template
                            :score (:composite (:score entry))
                            :summary (select-keys summary [:composite-score :avg-entropy-n :avg-change :temporal-autocorr])})
    {:label label :score (:composite (:score entry))}))

(defn run-exotype-pair! [idx entry]
  (let [label (format "mission-07-b-top%d" idx)
        ppm-nom (str out-dir "/" label "-nominate.ppm")
        ppm-lock (str out-dir "/" label "-locked.ppm")
        meta-nom (str out-dir "/" label "-nominate.edn")
        meta-lock (str out-dir "/" label "-locked.edn")
        seed (:seed entry)
        length (:length entry)
        generations (:generations entry)
        exo (exotype/resolve-exotype (:exotype entry))
        {:keys [genotype phenotype]} (start-state seed length)
        nominate (mmca/run-mmca {:genotype genotype
                                 :phenotype phenotype
                                 :generations generations
                                 :kernel :mutating-template
                                 :operators []
                                 ...
                                 :exotype-mode :nominate
                                 :seed seed})
        nomination (last (:exotype-nominations nominate))
        kernel (:kernel nomination)
        locked (mmca/run-mmca {:genotype genotype
                               :phenotype phenotype
                               :generations generations
                               :kernel kernel
                               :lock-kernel true
    ...
    (when-not nomination
      (throw (ex-info "No kernel nomination recorded." {:label label})))
    (render/render-run->file! nominate ppm-nom {:exotype? true})
    (render/render-run->file! locked ppm-lock {:exotype? true})
    (write-meta! meta-nom {:label label
                           :seed seed
                           :length length
                           :generations generations
                           :kernel :mutating-template
                           :exotype (select-keys exo [:sigil :tier :params])
                           :exotype-mode :nominate
                           :nomination (select-keys nomination [:tick :kernel-id])
                           :score (:final (:score entry))
                           :render {:exotype? true}})
    (write-meta! meta-lock {:label label
                            :seed seed
                            :length length
                            :generations generations
                            :kernel kernel
...
(def entries-a (load-entries "/tmp/mission-07-a.edn"))
(def entries-b (load-entries "/tmp/mission-07-b.edn"))

(def top-a (take 2 (sort-by (comp :composite :score) > entries-a)))
(def top-b (take 2 (sort-by (comp :final :score) > entries-b)))

(def a-summaries (mapv (fn [[idx entry]] (run-image-a! idx entry)) (map vector (range 1 3) top-a)))
(def b-summaries (mapv (fn [[idx entry]] (run-exotype-pair! idx entry)) (map vector (range 1 3) top-b)))

(println "Regime A top" a-summaries)
(println "Regime B top" b-summaries)'
#+END_SRC
...
figdir = Path('/home/joe/code/futon5/resources/figures')
for ppm in figdir.glob('mission-07-*.ppm'):
    img = Image.open(ppm)
    img.save(ppm.with_suffix('.png'))
PY
#+END_SRC
...
figdir = Path('/home/joe/code/futon5/resources/figures')
min_width = 1200
for path in figdir.glob('mission-07-*.png'):
    img = Image.open(path)
    if img.width >= min_width:
        continue
    scale = max(2, int((min_width + img.width - 1) // img.width))
    new_size = (img.width * scale, img.height * scale)
    img.resize(new_size, Image.NEAREST).save(path)
PY
#+END_SRC

#+CAPTION: Mission 7A top genotype #1 (score 45.93).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-a-top1.png]]

#+CAPTION: Mission 7A top genotype #2 (score 43.10).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-a-top2.png]]

#+CAPTION: Mission 7B top exotype #1 (sigil 兄, bits 11100100, super) nomination run.
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-nominate.png]]

#+CAPTION: Mission 7B top exotype #1 locked kernel rerun.
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-locked.png]]

#+CAPTION: Mission 7B top exotype #1 Lock-0 (kernel only, fresh rng).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-lock0.png]]

#+CAPTION: Mission 7B top exotype #1 Lock-1 (kernel + rng seed).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-lock1.png]]

#+CAPTION: Mission 7B top exotype #1 Lock-2 (kernel + context schedule).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-lock2.png]]

*** Lock notes (top exotype #1)
- Lock-0 is visibly noisier and shifts the exotype banding compared to the nomination run.
- Lock-2 most closely preserves the nomination structure, which suggests context replay is the critical stabilizer.

#+CAPTION: Mission 7B top exotype #1 lock comparison grid (nominate, lock-0, lock-1, lock-2).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-lock-grid.png]]

#+CAPTION: Mission 7B top exotype #2 (sigil 兄, bits 11100100, super) nomination run.
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-nominate.png]]

#+CAPTION: Mission 7B top exotype #2 locked kernel rerun.
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-locked.png]]

#+CAPTION: Mission 7B top exotype #2 Lock-0 (kernel only, fresh rng).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-lock0.png]]

#+CAPTION: Mission 7B top exotype #2 Lock-1 (kernel + rng seed).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-lock1.png]]

#+CAPTION: Mission 7B top exotype #2 Lock-2 (kernel + context schedule).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-lock2.png]]

*** Lock notes (top exotype #2)
- Lock-0/1/2 diverge in fine texture even with the same nominated kernel, indicating evaluator sampling noise matters.
- Lock-2 is the most faithful replay of the nomination regime; it preserves the exotype banding best among the locks.

#+CAPTION: Mission 7B top exotype #2 lock comparison grid (nominate, lock-0, lock-1, lock-2).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-lock-grid.png]]

*** Context-only baseline (top exotype #1)
#+BEGIN_SRC bash
bb -cp src:resources -e '(require (quote [clojure.java.io :as io])
                           (quote [futon5.ca.core :as ca])
                           (quote [futon5.mmca.exotype :as exotype])
...
(def label "mission-07-b-top1-context-only")
(def bits "11100100")
(def sigil (ca/sigil-for bits))
(def seed 4242)
(def length 50)
...
(let [{:keys [genotype phenotype]} (start-state seed length)
  ...
  (spit meta-path (pr-str {:label label
                           ...
                           :generations generations
                           :kernel :mutating-template
                           :exotype {:sigil sigil :bits bits :tier :super}
                           :exotype-mode :nominate
                           :exotype-context-mode :random
                           :render {:exotype? true}}))
  (println "context-only baseline written" ppm-path))'
#+END_SRC

#+CAPTION: Mission 7B top exotype #1 with randomized context sampling (context-only baseline).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top1-context-only.png]]

*** Context-only note
- The randomized-context baseline dampens structured bands while leaving the exotype stripe active, suggesting part of the evaluator signal is coming from context/phenotype cues rather than genotype structure alone.
- This supports keeping a context-only guardrail in the xenotype cull step to avoid evaluator shortcuts.

#+CAPTION: Mission 7B top exotype #2 with randomized context sampling (context-only baseline).
#+ATTR_ORG: :width 1200
[[file:figures/mission-07-b-top2-context-only.png]]

*** Mission 7 conclusion (draft)
- In the pilot, exotype-evolved evaluators shift the score distribution upward and shorten stasis relative to direct optimization, while exposing sensitivity to context sampling.
- Lock-2 replay preserves the nomination structure most faithfully, indicating that evaluator context is a meaningful driver of the observed regimes.
- Context-only baselines damp structured bands while leaving exotype stripes active, supporting a xenotype guardrail against evaluator shortcuts.
- Next decision: whether to scale the A/B runs (e.g., 1–5k evaluations) to stabilize the distributional claims before writing the final section.

*** Overnight run plan (batched + restartable)
Run in batches so partial results are preserved. Each batch writes its own log; concatenation is safe because entries are line-delimited EDN.
...
# Sanity check (expected lines = batches * runs_per_batch)
#+BEGIN_SRC bash
wc -l /tmp/mission-07-a-batch-*.edn /tmp/mission-07-b-batch-*.edn | tail -n 4
wc -l /tmp/mission-07-a-overnight.edn /tmp/mission-07-b-overnight.edn
#+END_SRC
...
(defn summarize [entries score-path]
  (let [scores (mapv #(get-in % score-path) entries)
        stasis (keep #(get-in % [:summary :first-stasis-step]) entries)
        overall {:count (count scores)
                 :mean (avg scores)
                 :min (apply min scores)
...
(def a (summarize (load-entries "/tmp/mission-07-a-overnight.edn") [:score :composite]))
(def b (summarize (load-entries "/tmp/mission-07-b-overnight.edn") [:score :final]))

(println "Overnight A" a)
(println "Overnight B" b)'
#+END_SRC

*** Overnight summary (A vs B)
| metric | Regime A | Regime B |
|--------+----------+----------|
| mean | 34.10 | 40.87 |
| q10 | 29.57 | 34.48 |
...
| q90 | 38.80 | 46.38 |
| max | 47.52 | 51.13 |
| confetti-rate | 0.0514 | 0.0508 |
| dead-rate | 0.00 | 0.00 |
| stasis-mean | 26.35 | 23.64 |

*** Overnight trend check (windowed)
#+BEGIN_SRC bash
bb -cp src:resources -e '(require (quote [clojure.edn :as edn])
                           (quote [clojure.string :as str]))
...
(defn summarize-window [entries score-path idx]
  (let [scores (mapv #(get-in % score-path) entries)]
    {:window idx
     :count (count scores)
     :mean (avg scores)
...
(defn windowed [entries score-path window-size]
  (->> (partition-all window-size entries)
       (map-indexed (fn [idx runs]
                      (summarize-window runs score-path (inc idx))))
       vec))

(def a (load-entries "/tmp/mission-07-a-overnight.edn"))
(def b (load-entries "/tmp/mission-07-b-overnight.edn"))

(def a-windows (windowed a [:score :composite] 100))
(def b-windows (windowed b [:score :final] 100))

(println "A windows" a-windows)
(println "B windows" b-windows)'
#+END_SRC

*** Assumption check (xenotype + CT layer)
- The overnight runs show stationary distributions rather than upward drift, implying xenotype is currently evaluative, not curriculum-shaping.
- The CT layer gives us a handle on program structure, but without a higher-order “ratchet” it doesn’t induce cumulative improvement.
- Next step: design a xenotype ratchet (memoryful + curriculum) that scores *improvement over windows* and/or enforces staged constraints, then evaluate by word class using the CT manifest.

*** Mission 7b (draft): Xenotype ratchet
Goal: turn the xenotype from a static evaluator into a curriculum-shaping ratchet that rewards *improvement over time*.

Core idea:
- Track scores over windows (e.g., 100 evals).
- Reward improvement between windows (delta‑score) rather than absolute score.
- Stage constraints (e.g., raise min diversity or lower confetti tolerance) as windows advance.

Protocol (pilot scale):
- Same A/B setup as Mission 7, but add a ratchet xenotype with:
  - window size = 100
  - ratchet target = +Δmean and +Δq50 vs previous window
  - penalty if confetti rate rises or diversity collapses
  - curriculum schedule: tighten confetti/entropy thresholds every 5 windows
- Measure:
  - drift slope (mean/q50 over windows)
  - stability of improvements across seeds
  - word‑class outcomes (CT manifest) under ratchet vs baseline

Acceptance signal:
- Demonstrable upward trend over windows *and* reduced regression when the ratchet tightens.

CT framing:
- Exotypes are programs (morphisms in `:futon5/exotype-program`).
- Xenotype evaluates families of exotype programs (meta-morphism / functor to score space).
- AIF analogy: xenotype as model-of-self-in-niche, preserving viability without prescribing targets.

** Mission 8 (draft): Local vs recursive-local evaluators
- Goal: test whether bounded recursive locality extends discoverability without collapsing into oracle-like behavior.
- Scale: match Mission 7 pilot (runs 200, pop 16, update cadence 100, length 50, generations 30).

*** Protocol (pilot scale)
Regimes:
- Regime L (local): current exotype evaluator (single context sample).
- Regime R (recursive-local): context sampling chained for depth d (e.g., d=2, d=3) via `:exotype-context-depth`.

Implementation sketch:
- Add a recursion depth option for context sampling (sample context, then sample from the implied next-gen history window).
- Keep all other parameters identical to Regime L so we isolate the effect of recursive locality.

Measurements:
- Non-degeneracy: dead/confetti rates, short-cycle detection.
- Discriminative signal: score variance across genotypes/exotypes.
- Open-ended dynamics: long transients and metastable bands (visual + summary stats).
- Stability: nomination vs locked rerun gap under Lock-2.

Acceptance signal:
- Recursive locality improves discoverability (higher score variance + richer bands) without a confetti spike.
...
bb -cp src:resources -m futon5.mmca.exoevolve \
  ...
  --tier both \
  --seed 4242 \
  --log /tmp/mission-08-local.edn

# recursive-local (depth 2)
bb -cp src:resources -m futon5.mmca.exoevolve \
  --runs 200 \
  --length 50 \
  --generations 30 \
  ...
  --seed 4242 \
  --context-depth 2 \
  --log /tmp/mission-08-rec2.edn

# recursive-local (depth 3)
bb -cp src:resources -m futon5.mmca.exoevolve \
  --runs 200 \
  --length 50 \
  --generations 30 \
  ...
  --seed 4242 \
  --context-depth 3 \
  --log /tmp/mission-08-rec3.edn
#+END_SRC

*** Pilot summary (local vs recursive-local)
| metric | local | rec2 | rec3 |
|--------+-------+------+------|
| mean | 32.68 | 32.76 | 33.18 |
| q10 | 27.03 | 24.86 | 27.04 |
| q50 | 32.74 | 33.12 | 33.73 |
| q90 | 39.28 | 39.73 | 38.86 |
| max | 48.64 | 51.18 | 48.35 |
| confetti-rate | 0.07 | 0.07 | 0.025 |
| dead-rate | 0.00 | 0.00 | 0.00 |
| stasis-mean | 20.40 | 17.85 | 18.94 |

*** Mission 8 CA printouts (top exemplars)
#+CAPTION: Mission 8 local top exotype (sigil 专, bits 01001110, super) nomination run.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-local-top1-nominate.png]]

#+CAPTION: Mission 8 local top exotype locked kernel rerun.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-local-top1-locked.png]]

#+CAPTION: Mission 8 rec2 top exotype (sigil 无, bits 01001011, super) nomination run.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-rec2-top1-nominate.png]]

#+CAPTION: Mission 8 rec2 top exotype locked kernel rerun.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-rec2-top1-locked.png]]

#+CAPTION: Mission 8 rec3 top exotype (sigil 尤, bits 01011100, local) nomination run.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-rec3-top1-nominate.png]]

#+CAPTION: Mission 8 rec3 top exotype locked kernel rerun.
#+ATTR_ORG: :width 1200
[[file:figures/mission-08-rec3-top1-locked.png]]

*** Mission 8 note
- Genotype/exotype bands are visually rich, but phenotype rows look comparatively flat in these exemplars.
- This suggests phenotype stability may lag behind evaluator richness; recursive-locality may need stronger phenotype stabilization to translate into visible phenotype complexity.
