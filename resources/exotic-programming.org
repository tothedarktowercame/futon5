...
** Future tickets
- Add optional curriculum gating: flag (e.g., `--curriculum-gate`) to penalize or null ratchet deltas below the threshold; keep observational mode as default.
...
- Implementation note: add `:context-depth` support in exotype context sampling + CLI flag in `exoevolve` to enable the pilot.
...
*** Supplementary 3: Exotype program word rewrites (Regime C)
- Purpose: Define and implement word-level rewrite operators for exotype programs (insert/remove `gate-update`, swap/toggle `set-mix`, move `normalize` under invariants).
- Output: Regime C mutation hook + logging of word classes so Mission 7 can compare A/B/C once enabled.
