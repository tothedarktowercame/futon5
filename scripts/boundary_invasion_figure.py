...
ARMS = [
    ("policy",        "policy alone"),
    ("invade-adopt",  "adopt from boundary"),
    ("invade-mutate", "mutate at pure locales"),
    ("yoked",         "yoked control"),
]

def load(d, arm, expect=3000):
    p = os.path.join(d, f"binv-{arm}-phe.txt")
    rows = [l.rstrip('\n') for l in open(p)]
...
def main(d, out):
    ...
    styles = {"policy": (RED, 'solid'), "invade-adopt": (RED, (0, (4, 2))),
              "invade-mutate": (LIVE, 'solid'), "yoked": (LIVE, (0, (4, 2)))}
    short = {"policy": "policy", "invade-adopt": "adopt",
             "invade-mutate": "mutate", "yoked": "yoked"}
    for arm, title in ARMS:
        ...
        axt.annotate(short[arm], (ts[-1], data[arm]['traj'][-1]),
                     xytext=(7, {"policy": 5, "invade-adopt": -6,
                                 "invade-mutate": -8, "yoked": 7}[arm]),
    ...
    fig.suptitle('Local intervention at the freezing arm ($\\beta$=16, $\\kappa$=0.1)',
                 fontsize=16, y=0.978, color=INK)
    fig.text(0.5, 0.945,
             'Frozen := unchanged for 15 steps.  Seed 2026102000.  '
             'Overrides begin at t=40: one operator write per firing, '
             'pure-locale cells on a jittered [15,25] dwell.',
    ...
    for ext in ('png', 'pdf'):
        plt.savefig(os.path.join(out, 'binv-rescue.' + ext), dpi=300,
                    facecolor=SURFACE)
    plt.close()
    print("wrote binv-rescue.{png,pdf}")
    for arm, _ in ARMS:
